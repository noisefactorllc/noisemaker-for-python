...
def _transpile(effect_id, program):
    ...
    defines = {s["define"]: ("float" if s.get("type") == "float" else "int")
               for s in eff["params"].values() if isinstance(s, dict) and s.get("define")}
...
def test_pipeline_solid_compiles_and_renders():
    ...
    ctx = Ctx(Runtime(), uniforms={"color": np.array([0.25, 0.5, 0.75], dtype=F32), "alpha": 1.0},
