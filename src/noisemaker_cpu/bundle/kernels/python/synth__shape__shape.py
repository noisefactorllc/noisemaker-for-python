def run_pixel(ctx, out):
    ...
    def circles__vec2_float(st, freq):
        st = rt.copy(st, "float")
        dist = rt.length(rt.binary("-", st, rt.array([rt.binary("*", rt.f(0.5), g.aspectRatio, 1, "float"), rt.f(0.5)]), 2, "float"))
    ...
    def shape__vec2_int_float(st, sides, blend):
        st = rt.copy(st, "float")
        st[:] = rt.binary("-", rt.binary("*", st, rt.f(2.0), 2, "float"), rt.array([g.aspectRatio, rt.f(1.0)]), 2, "float")
