def run_pixel(ctx, out):
    ...
    def hueDistance__float_float(h1, h2):
        d = rt.component_wise("abs", rt.binary("-", h1, h2, 1), width=1)
        return rt.component_wise("min", d, rt.binary("-", rt.f(1.0), d, 1), width=1)
    def main__void():
        ...
        inner = _u_range
        outer = rt.binary("+", _u_range, _u_feather, 1)
        mask = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", inner, outer, dist, width=1), 1)
        mask = rt.binary("*", mask, sat, 1)
