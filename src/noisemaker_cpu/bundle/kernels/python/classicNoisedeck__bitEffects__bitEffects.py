def run_pixel(ctx, out):
    ...
    g.BIT_COUNT = rt.i(8)
    g.mask = rt.binary("-", rt.binary("<<", rt.i(1), g.BIT_COUNT, 1), rt.i(1), 1)
    ...
    def rotate2D__vec2_float(st, rot):
        ...
        angle = rt.binary("*", rot, rt.f(6.28318530718), 1)
        st = rt.binary("-", st, rt.binary("*", _u_fullResolution, rt.f(0.5), 2), 2)
        st = rt.binary("*", rt.construct(4, rt.component_wise("cos", angle, width=1), rt.unary("-", rt.component_wise("sin", angle, width=1)), rt.component_wise("sin", angle, width=1), rt.component_wise("cos", angle, width=1)), st, 4)
        st = rt.binary("+", st, rt.binary("*", _u_fullResolution, rt.f(0.5), 2), 2)
        return st
    def periodicFunction__float(p):
        return map__float_float_float_float_float(rt.component_wise("sin", rt.binary("*", p, rt.f(6.28318530718), 1), width=1), rt.unary("-", rt.f(1.0)), rt.f(1.0), rt.f(0.0), rt.f(1.0))
    def randomFromLatticeWithOffset__vec2_float_float_float_ivec2(st, xFreq, yFreq, s, offset):
        st = rt.copy(st)
        offset = rt.copy(offset)
        lattice = rt.construct(2, rt.binary("*", rt.swizzle(st, "x"), xFreq, 1), rt.binary("*", rt.swizzle(st, "y"), yFreq, 1))
        baseFloor = rt.component_wise("floor", lattice, width=2)
        base = rt.binary("+", cpu_ivec2__vec2(baseFloor), offset, 2)
        frac = rt.binary("-", lattice, baseFloor, 2)
        seedInt = rt.construct(1, rt.component_wise("floor", s, width=1))
        ...
        seedBits = rt.component_wise("floatBitsToUint", s, width=1)
        fracBits = rt.component_wise("floatBitsToUint", seedFrac, width=1)
    ...
    def constant__vec2_float_float_float(st, xFreq, yFreq, s):
        ...
        scaledTime = rt.binary("*", periodicFunction__float(rt.binary("-", rt.swizzle(randTime, "x"), _u_time, 1)), map__float_float_float_float_float(rt.component_wise("abs", _u_speed, width=1), rt.f(0.0), rt.f(100.0), rt.f(0.0), rt.f(0.333)), 1)
        rand = randomFromLatticeWithOffset__vec2_float_float_float_ivec2(st, xFreq, yFreq, s, cpu_ivec2__float_float(rt.i(0), rt.i(0)))
        return periodicFunction__float(rt.binary("-", rt.swizzle(rand, "x"), scaledTime, 1))
    def value__vec2_float_float_float(st, xFreq, yFreq, s):
        ...
        if rt.binary("==", _u_INTERP, rt.i(0)):
            ...
            ndX = rt.binary("/", rt.f(1.0), xFreq, 1)
            ndY = rt.binary("/", rt.f(1.0), yFreq, 1)
            x1y2 = constant__vec2_float_float_float(rt.construct(2, rt.swizzle(st, "x"), rt.binary("+", rt.swizzle(st, "y"), ndY, 1)), xFreq, yFreq, s)
            x2y1 = constant__vec2_float_float_float(rt.construct(2, rt.binary("+", rt.swizzle(st, "x"), ndX, 1), rt.swizzle(st, "y")), xFreq, yFreq, s)
            x2y2 = constant__vec2_float_float_float(rt.construct(2, rt.binary("+", rt.swizzle(st, "x"), ndX, 1), rt.binary("+", rt.swizzle(st, "y"), ndY, 1)), xFreq, yFreq, s)
            uv = rt.construct(2, rt.binary("*", rt.swizzle(st, "x"), xFreq, 1), rt.binary("*", rt.swizzle(st, "y"), yFreq, 1))
    ...
    def modi__int_int(x, y):
        return rt.binary("&", rt.binary("%", x, y, 1), g.mask, 1)
    def _or__int_int(a, b):
        return rt.binary("|", rt.binary("&", a, g.mask, 1), rt.binary("&", b, g.mask, 1), 1)
    def _and__int_int(a, b):
        return rt.binary("&", rt.binary("&", a, g.mask, 1), rt.binary("&", b, g.mask, 1), 1)
    def not2__int(a):
        return rt.binary("&", rt.binary("^", a, rt.i(0xFFFFFFF), 1), g.mask, 1)
    def xor__int_int(a, b):
        return rt.binary("^", rt.binary("&", a, g.mask, 1), rt.binary("&", b, g.mask, 1), 1)
    def _or__float_float(a, b):
        return rt.construct(1, _or__int_int(rt.construct(1, a), rt.construct(1, b)))
    def _and__float_float(a, b):
        return rt.construct(1, _and__int_int(rt.construct(1, a), rt.construct(1, b)))
    def not3__float(a):
        return rt.construct(1, not2__int(rt.construct(1, a)))
    def xor__float_float(a, b):
        return rt.construct(1, xor__int_int(rt.construct(1, a), rt.construct(1, b)))
    def bitValue__vec2_float_float(st, freq, nForColor):
        st = rt.copy(st)
        blendy = rt.binary("+", nForColor, rt.binary("*", periodicFunction__float(rt.binary("*", value__vec2_float_float_float(st, rt.binary("*", freq, rt.f(0.01), 1), rt.binary("*", freq, rt.f(0.01), 1), nForColor), rt.f(0.1), 1)), rt.f(100.0), 1), 1)
        v = rt.f(1.0)
        if rt.binary("==", _u_FORMULA, rt.i(0)):
            v = rt.component_wise("mod", xor__float_float(rt.binary("*", rt.swizzle(st, "x"), freq, 1), rt.binary("*", rt.swizzle(st, "y"), freq, 1)), blendy, width=1)
        else:
            if rt.binary("==", _u_FORMULA, rt.i(1)):
                v = rt.component_wise("mod", _or__float_float(rt.binary("*", rt.swizzle(st, "x"), freq, 1), rt.binary("*", rt.swizzle(st, "y"), freq, 1)), blendy, width=1)
            else:
                if rt.binary("==", _u_FORMULA, rt.i(2)):
                    v = rt.component_wise("mod", rt.binary("*", rt.binary("*", rt.swizzle(st, "x"), freq, 1), rt.binary("*", rt.swizzle(st, "y"), freq, 1), 1), blendy, width=1)
                else:
                    if rt.binary("==", _u_FORMULA, rt.i(3)):
                        v = rt.construct(1, rt.binary("<", xor__float_float(rt.binary("*", rt.swizzle(st, "x"), freq, 1), rt.binary("*", rt.swizzle(st, "y"), freq, 1)), blendy))
                    else:
                        if rt.binary("==", _u_FORMULA, rt.i(4)):
                            v = rt.component_wise("mod", rt.binary("*", rt.binary("*", rt.swizzle(st, "x"), freq, 1), blendy, 1), rt.binary("*", rt.swizzle(st, "y"), freq, 1), width=1)
                        else:
                            if rt.binary("==", _u_FORMULA, rt.i(5)):
                                v = rt.component_wise("mod", rt.binary("*", rt.binary("-", rt.binary("*", rt.swizzle(st, "x"), freq, 1), rt.f(0.5), 1), rt.f(0.25), 1), rt.binary("-", rt.binary("*", rt.swizzle(st, "y"), freq, 1), rt.f(0.5), 1), width=1)
                            ...
                            if rt.binary("==", _u_COLOR_SCHEME, rt.i(5)):
                                ...
                                if rt.binary("==", _u_COLOR_SCHEME, rt.i(6)):
                                    ...
                                    if rt.binary("==", _u_COLOR_SCHEME, rt.i(10)):
                                        color = rt.assign_swizzle(color, "b", bitValue__vec2_float_float(st, freq, _u_n))
                                        color = rt.assign_swizzle(color, "g", bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(1.0), 1)))
                                    else:
                                        if rt.binary("==", _u_COLOR_SCHEME, rt.i(11)):
                                            color = rt.assign_swizzle(color, "b", bitValue__vec2_float_float(st, freq, _u_n))
                                            color = rt.assign_swizzle(color, "r", bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(1.0), 1)))
                                        else:
                                            if rt.binary("==", _u_COLOR_SCHEME, rt.i(12)):
                                                color = rt.assign_swizzle(color, "b", bitValue__vec2_float_float(st, freq, _u_n))
                                                color = rt.assign_swizzle(color, "rg", rt.construct(2, bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(1.0), 1))))
                                            else:
                                                if rt.binary("==", _u_COLOR_SCHEME, rt.i(13)):
                                                    color = rt.assign_swizzle(color, "g", bitValue__vec2_float_float(st, freq, _u_n))
                                                    color = rt.assign_swizzle(color, "rb", rt.construct(2, bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(1.0), 1))))
                                                else:
                                                    if rt.binary("==", _u_COLOR_SCHEME, rt.i(14)):
                                                        ...
                                                        if rt.binary("==", _u_COLOR_SCHEME, rt.i(15)):
                                                            color = rt.assign_swizzle(color, "r", bitValue__vec2_float_float(st, freq, _u_n))
                                                            color = rt.assign_swizzle(color, "bg", rt.construct(2, bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(1.0), 1))))
                                                        else:
                                                            if rt.binary("==", _u_COLOR_SCHEME, rt.i(20)):
                                                                color = rt.assign_swizzle(color, "r", bitValue__vec2_float_float(st, freq, _u_n))
                                                                color = rt.assign_swizzle(color, "g", bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(1.0), 1)))
                                                                color = rt.assign_swizzle(color, "b", bitValue__vec2_float_float(st, freq, rt.binary("+", _u_n, rt.f(2.0), 1)))
        return color
    ...
    def arecibo__vec2_float_float_float(st, xFreq, yFreq, _seed):
        st = rt.copy(st)
        xMod = rt.component_wise("mod", rt.component_wise("floor", rt.binary("*", rt.swizzle(st, "x"), xFreq, 1), width=1), xFreq, width=1)
        yMod = rt.component_wise("mod", rt.component_wise("floor", rt.binary("*", rt.swizzle(st, "y"), yFreq, 1), width=1), yFreq, width=1)
        v = rt.f(1.0)
        if rt.binary("||", rt.binary("==", xMod, rt.f(0.0)), rt.binary("||", rt.binary("==", yMod, rt.f(0.0)), rt.binary("||", rt.binary("==", xMod, rt.binary("-", xFreq, rt.f(1.0), 1)), rt.binary("==", yMod, rt.binary("-", yFreq, rt.f(1.0), 1))))):
            v = rt.f(0.0)
        else:
    ...
    def areciboNum__vec2_float_float(st, freq, _seed):
        st = rt.copy(st)
        return arecibo__vec2_float_float_float(st, rt.binary("+", rt.component_wise("floor", rt.binary("*", freq, rt.f(0.5), 1), width=1), rt.f(1.0), 1), rt.component_wise("floor", freq, width=1), _seed)
    def glyphs__vec2_float_float(st, freq, _seed):
        st = rt.copy(st)
        xFreq = rt.component_wise("floor", rt.binary("*", freq, rt.f(0.75), 1), width=1)
        xMod = rt.component_wise("mod", rt.component_wise("floor", rt.binary("*", rt.swizzle(st, "x"), xFreq, 1), width=1), xFreq, width=1)
        yMod = rt.component_wise("mod", rt.component_wise("floor", rt.binary("*", rt.swizzle(st, "y"), freq, 1), width=1), freq, width=1)
        v = rt.f(1.0)
        if rt.binary("||", rt.binary("==", xMod, rt.f(0.0)), rt.binary("||", rt.binary("==", yMod, rt.f(0.0)), rt.binary("||", rt.binary("==", xMod, rt.binary("-", xFreq, rt.f(1.0), 1)), rt.binary("==", yMod, rt.binary("-", freq, rt.f(1.0), 1))))):
            v = rt.f(0.0)
        else:
    ...
    def invaders__vec2_float_float(st, freq, _seed):
        st = rt.copy(st)
        xMod = rt.component_wise("mod", rt.component_wise("floor", rt.binary("*", rt.swizzle(st, "x"), freq, 1), width=1), freq, width=1)
        yMod = rt.component_wise("mod", rt.component_wise("floor", rt.binary("*", rt.swizzle(st, "y"), freq, 1), width=1), freq, width=1)
        v = rt.f(1.0)
        if rt.binary("||", rt.binary("==", xMod, rt.f(0.0)), rt.binary("||", rt.binary("==", yMod, rt.f(0.0)), rt.binary("||", rt.binary("==", xMod, rt.binary("-", freq, rt.f(1.0), 1)), rt.binary("==", yMod, rt.binary("-", freq, rt.f(1.0), 1))))):
            v = rt.f(0.0)
        else:
            if rt.binary(">=", xMod, rt.binary("*", freq, rt.f(0.5), 1)):
                v = maskValue__vec2_float_float(rt.construct(2, rt.binary("+", rt.component_wise("floor", rt.swizzle(st, "x"), width=1), rt.binary("-", rt.f(1.0), rt.component_wise("fract", rt.swizzle(st, "x"), width=1), 1), 1), rt.swizzle(st, "y")), freq, _seed)
            else:
    ...
    def bitMask__vec2(st):
        ...
        st = rt.binary("+", st, rt.construct(2, rt.binary("/", rt.binary("*", rt.f(0.5), rt.swizzle(_u_fullResolution, "x"), 1), rt.swizzle(_u_fullResolution, "y"), 1), rt.f(0.5)), 2)
        st = rt.assign_swizzle(st, "x", rt.binary("-", rt.swizzle(st, "x"), rt.binary("/", rt.binary("*", rt.f(0.5), rt.swizzle(_u_fullResolution, "x"), 1), rt.swizzle(_u_fullResolution, "y"), 1), 1))
        ...
        if rt.binary("==", _u_MASK_COLOR_SCHEME, rt.i(0)):
            ...
            baseHue = rt.binary("+", rt.f(0.01), rt.binary("*", rt.binary("*", maskValue__vec2_float_float(st, rt.f(1.0), rt.unary("-", rt.f(100.0))), _u_baseHueRange, 1), rt.f(0.01), 1), 1)
            color = rt.assign_swizzle(color, "r", rt.binary("*", rt.component_wise("fract", rt.binary("+", rt.binary("+", baseHue, rt.binary("*", rt.binary("*", bitMaskValue__vec2_float_float(st, freq, rt.f(0.0)), _u_hueRange, 1), rt.f(0.01), 1), 1), rt.binary("-", rt.f(1.0), rt.binary("/", _u_hueRotation, rt.f(360.0), 1), 1), 1), width=1), mask, 1))
            if rt.binary("==", _u_MASK_COLOR_SCHEME, rt.i(3)):
                color = rt.assign_swizzle(color, "g", mask)
            else:
                color = rt.assign_swizzle(color, "g", rt.binary("*", bitMaskValue__vec2_float_float(st, freq, rt.f(25.0)), mask, 1))
            if rt.binary("||", rt.binary("==", _u_MASK_COLOR_SCHEME, rt.i(2)), rt.binary("==", _u_MASK_COLOR_SCHEME, rt.i(3))):
                color = rt.assign_swizzle(color, "b", mask)
            else:
                color = rt.assign_swizzle(color, "b", rt.binary("*", bitMaskValue__vec2_float_float(st, freq, rt.f(50.0)), mask, 1))
    ...
    def main__void():
        ...
        if rt.binary("==", _u_MODE, rt.i(0)):
            ...
            st = rt.binary("+", st, rt.binary("+", _u_seed, rt.f(1000.0), 1), 2)
