def run_pixel(ctx, out):
        ...
        for _for0 in range(1048576):
            ...
            for _for1 in range(1048576):
                ...
                if not (rt.binary("<=", x, rt.i(1))):
                    break
                cell = rt.construct(2, x, y)
                pt = rt.binary("+", rt.binary("+", cell, rt.f(0.5), 2), rt.binary("*", rt.binary("-", hash22__vec2(rt.binary("+", rt.binary("+", g, cell, 2), rt.binary("*", seedVal, rt.f(101.7), 1), 2)), rt.f(0.5), 2), jitter, 2), 2)
                d = rt.dot(rt.binary("-", pt, f, 2), rt.binary("-", pt, f, 2))
                if rt.binary("<", d, best):
                    second = best
                    best = d
                else:
    ...
    def reliefShade__float_float_float_float_float(hC, hR, hT, strength, lightAngleDeg):
        grad = rt.binary("*", rt.construct(2, rt.binary("-", hR, hC, 1), rt.binary("-", hT, hC, 1)), strength, 2)
    ...
    def crackMask__vec2_float_float_float(gc, spacingPx, depthPct, seedVal):
        gc = rt.copy(gc)
        wob = rt.binary("*", rt.construct(2, vnoise__vec2(rt.binary("/", gc, rt.f(6.0), 2)), vnoise__vec2(rt.binary("+", rt.binary("/", gc, rt.f(6.0), 2), rt.construct(2, rt.f(37.7), rt.f(91.3)), 2))), rt.f(2.0), 2)
        p = rt.binary("/", rt.binary("+", gc, wob, 2), spacingPx, 2)
        f1f2 = voronoiF1F2__vec2_float_float(p, rt.f(1.0), seedVal)
        d = rt.binary("*", rt.binary("-", rt.swizzle(f1f2, "y"), rt.swizzle(f1f2, "x"), 1), spacingPx, 1)
        edge = rt.binary("+", rt.f(1.5), rt.binary("*", rt.binary("/", depthPct, rt.f(100.0), 1), rt.f(2.0), 1), 1)
        return rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.0), edge, d, width=1), 1)
    def main__void():
        ...
        kR = crackMask__vec2_float_float_float(rt.binary("+", globalCoord, rt.construct(2, rt.f(1.0), rt.f(0.0)), 2), _u_spacing, _u_depth, seedF)
        kL = crackMask__vec2_float_float_float(rt.binary("-", globalCoord, rt.construct(2, rt.f(1.0), rt.f(0.0)), 2), _u_spacing, _u_depth, seedF)
        kT = crackMask__vec2_float_float_float(rt.binary("+", globalCoord, rt.construct(2, rt.f(0.0), rt.f(1.0)), 2), _u_spacing, _u_depth, seedF)
        kB = crackMask__vec2_float_float_float(rt.binary("-", globalCoord, rt.construct(2, rt.f(0.0), rt.f(1.0)), 2), _u_spacing, _u_depth, seedF)
        gradK = rt.construct(2, rt.binary("*", rt.binary("-", kR, kL, 1), rt.f(0.5), 1), rt.binary("*", rt.binary("-", kT, kB, 1), rt.f(0.5), 1))
        hC = rt.unary("-", kC)
        hR = rt.binary("-", hC, rt.swizzle(gradK, "x"), 1)
        hT = rt.binary("-", hC, rt.swizzle(gradK, "y"), 1)
        ...
        shadeMul = rt.binary("+", rt.f(1.0), rt.binary("*", rt.binary("*", rt.binary("*", rt.binary("-", shade, rt.f(0.6), 1), rt.f(2.0), 1), rt.binary("/", rt.binary("*", rt.f(0.25), _u_depth, 1), rt.f(100.0), 1), 1), wallMask, 1), 1)
        darkened = rt.binary("*", rt.swizzle(src, "rgb"), rt.component_wise("mix", rt.f(1.0), rt.binary("+", rt.f(0.35), rt.binary("*", rt.binary("/", _u_brightness, rt.f(100.0), 1), rt.f(0.5), 1), 1), kC, width=1), 3)
        result = rt.component_wise("clamp", rt.binary("*", darkened, shadeMul, 3), rt.f(0.0), rt.f(1.0), width=3)
