def run_pixel(ctx, out):
    ...
    def hue2rgb__float_float_float(p, q, t):
        if rt.binary("<", t, rt.f(0.0)):
            t = rt.binary("+", t, rt.f(1.0), 1)
        if rt.binary(">", t, rt.f(1.0)):
            t = rt.binary("-", t, rt.f(1.0), 1)
        if rt.binary("<", t, rt.binary("/", rt.f(1.0), rt.f(6.0), 1)):
            return rt.binary("+", p, rt.binary("*", rt.binary("*", rt.binary("-", q, p, 1), rt.f(6.0), 1), t, 1), 1)
        if rt.binary("<", t, rt.binary("/", rt.f(1.0), rt.f(2.0), 1)):
            return q
        if rt.binary("<", t, rt.binary("/", rt.f(2.0), rt.f(3.0), 1)):
            return rt.binary("+", p, rt.binary("*", rt.binary("*", rt.binary("-", q, p, 1), rt.binary("-", rt.binary("/", rt.f(2.0), rt.f(3.0), 1), t, 1), 1), rt.f(6.0), 1), 1)
        return p
    def hslToRgb__vec3(hsl):
        hsl = rt.copy(hsl)
        if rt.binary("==", rt.swizzle(hsl, "y"), rt.f(0.0)):
            return rt.construct(3, rt.swizzle(hsl, "z"))
        q = (rt.binary("*", rt.swizzle(hsl, "z"), rt.binary("+", rt.f(1.0), rt.swizzle(hsl, "y"), 1), 1) if rt.binary("<", rt.swizzle(hsl, "z"), rt.f(0.5)) else rt.binary("-", rt.binary("+", rt.swizzle(hsl, "z"), rt.swizzle(hsl, "y"), 1), rt.binary("*", rt.swizzle(hsl, "z"), rt.swizzle(hsl, "y"), 1), 1))
        p = rt.binary("-", rt.binary("*", rt.f(2.0), rt.swizzle(hsl, "z"), 1), q, 1)
        return rt.construct(3, hue2rgb__float_float_float(p, q, rt.binary("+", rt.swizzle(hsl, "x"), rt.binary("/", rt.f(1.0), rt.f(3.0), 1), 1)), hue2rgb__float_float_float(p, q, rt.swizzle(hsl, "x")), hue2rgb__float_float_float(p, q, rt.binary("-", rt.swizzle(hsl, "x"), rt.binary("/", rt.f(1.0), rt.f(3.0), 1), 1)))
    def luma__vec3(rgb):
        rgb = rt.copy(rgb)
    ...
    def lutWarmFilm__vec3(rgb):
        rgb = rt.copy(rgb)
        rgb = rt.binary("+", rt.binary("*", rgb, rt.f(0.95), 3), rt.f(0.05), 3)
        ...
        l = luma__vec3(rgb)
        rgb = rt.assign_swizzle(rgb, "g", rt.component_wise("mix", rt.binary("*", rt.swizzle(rgb, "g"), rt.f(0.95), 1), rt.swizzle(rgb, "g"), l, width=1))
        rgb = rt.binary("*", rt.binary("*", rgb, rgb, 3), rt.binary("-", rt.f(3.0), rt.binary("*", rt.f(2.0), rgb, 3), 3), 3)
        return rgb
    def lutCoolShadows__vec3(rgb):
        rgb = rt.copy(rgb)
        l = luma__vec3(rgb)
        coolBlue = rt.construct(3, rt.f(0.4), rt.f(0.5), rt.f(0.7))
        shadowMask = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.0), rt.f(0.5), l, width=1), 1)
        rgb = rt.component_wise("mix", rgb, rt.binary("*", rt.binary("*", coolBlue, l, 3), rt.f(2.0), 3), rt.binary("*", shadowMask, rt.f(0.4), 1), width=3)
        return rgb
    def lutBleachBypass__vec3(rgb):
        ...
        rgb = rt.binary("+", rt.binary("*", rt.binary("-", rgb, rt.f(0.5), 3), rt.f(1.3), 3), rt.f(0.5), 3)
        rgb = rt.assign_swizzle(rgb, "r", rt.binary("*", rt.swizzle(rgb, "r"), rt.f(1.02), 1))
        rgb = rt.assign_swizzle(rgb, "b", rt.binary("*", rt.swizzle(rgb, "b"), rt.f(0.98), 1))
    ...
    def lutCrossProcess__vec3(rgb):
        ...
        l = luma__vec3(rgb)
        rgb = rt.assign_swizzle(rgb, "r", rt.binary("+", rt.swizzle(rgb, "r"), rt.binary("+", rt.binary("*", rt.binary("-", rt.f(1.0), l, 1), rt.unary("-", rt.f(0.1)), 1), rt.binary("*", l, rt.f(0.1), 1), 1), 1))
        rgb = rt.assign_swizzle(rgb, "g", rt.binary("+", rt.swizzle(rgb, "g"), rt.binary("*", rt.binary("-", rt.f(1.0), l, 1), rt.f(0.05), 1), 1))
        rgb = rt.assign_swizzle(rgb, "b", rt.binary("+", rt.swizzle(rgb, "b"), rt.binary("+", rt.binary("*", rt.binary("-", rt.f(1.0), l, 1), rt.f(0.1), 1), rt.binary("*", l, rt.unary("-", rt.f(0.15)), 1), 1), 1))
        hsl = rgbToHsl__vec3(rgb)
        hsl = rt.assign_swizzle(hsl, "y", rt.binary("*", rt.swizzle(hsl, "y"), rt.f(1.2), 1))
        rgb = hslToRgb__vec3(hsl)
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutCinematic__vec3(rgb):
        rgb = rt.copy(rgb)
        l = luma__vec3(rgb)
        rgb = rt.binary("+", rt.binary("*", rgb, rt.f(0.9), 3), rt.f(0.03), 3)
        shadowTint = rt.construct(3, rt.f(0.95), rt.f(1.0), rt.f(1.05))
        highlightTint = rt.construct(3, rt.f(1.05), rt.f(1.0), rt.f(0.95))
        rgb = rt.binary("*", rgb, rt.component_wise("mix", shadowTint, highlightTint, l, width=3), 3)
        rgb = rt.component_wise("pow", rgb, rt.construct(3, rt.f(1.1)), width=3)
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutDayForNight__vec3(rgb):
        rgb = rt.copy(rgb)
        l = luma__vec3(rgb)
        rgb = rt.assign_swizzle(rgb, "r", rt.binary("*", rt.swizzle(rgb, "r"), rt.f(0.5), 1))
        rgb = rt.assign_swizzle(rgb, "g", rt.binary("*", rt.swizzle(rgb, "g"), rt.f(0.6), 1))
        rgb = rt.assign_swizzle(rgb, "b", rt.binary("*", rt.swizzle(rgb, "b"), rt.f(1.0), 1))
        rgb = rt.binary("*", rgb, rt.f(0.4), 3)
        rgb = rt.component_wise("mix", rt.construct(3, luma__vec3(rgb)), rgb, rt.f(0.7), width=3)
        return rgb
    def lutVintage__vec3(rgb):
        rgb = rt.copy(rgb)
        rgb = rt.binary("+", rt.binary("*", rgb, rt.f(0.85), 3), rt.f(0.08), 3)
        rgb = rt.assign_swizzle(rgb, "r", rt.component_wise("pow", rt.swizzle(rgb, "r"), rt.f(0.95), width=1))
        rgb = rt.assign_swizzle(rgb, "b", rt.component_wise("pow", rt.swizzle(rgb, "b"), rt.f(1.1), width=1))
        hsl = rgbToHsl__vec3(rgb)
        hsl = rt.assign_swizzle(hsl, "y", rt.binary("*", rt.swizzle(hsl, "y"), rt.f(0.7), 1))
        rgb = hslToRgb__vec3(hsl)
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutNoir__vec3(rgb):
        rgb = rt.copy(rgb)
        l = luma__vec3(rgb)
        l = rt.binary("+", rt.binary("*", rt.binary("-", l, rt.f(0.5), 1), rt.f(1.5), 1), rt.f(0.5), 1)
        l = rt.component_wise("clamp", l, rt.f(0.0), rt.f(1.0), width=1)
        blue = rt.construct(3, rt.f(0.9), rt.f(0.95), rt.f(1.0))
        mono = rt.binary("*", rt.construct(3, l), rt.component_wise("mix", blue, rt.construct(3, rt.f(1.0)), l, width=3), 3)
    ...
    def lutSepia__vec3(rgb):
        ...
        result = rt.binary("*", l, sepia, 3)
        result = rt.binary("+", rt.binary("*", result, rt.f(0.9), 3), rt.f(0.05), 3)
    ...
    def lutInfrared__vec3(rgb):
        ...
        result = rt.assign_swizzle(result, "g", rt.binary("*", rt.swizzle(rgb, "g"), rt.f(0.3), 1))
        result = rt.assign_swizzle(result, "b", rt.binary("-", rt.f(1.0), l, 1))
        foliage = rt.binary("*", rt.component_wise("smoothstep", rt.f(0.2), rt.f(0.6), rt.swizzle(rgb, "g"), width=1), rt.binary("-", rt.f(1.0), rt.component_wise("abs", rt.binary("-", rt.swizzle(rgb, "r"), rt.swizzle(rgb, "b"), 1), width=1), 1), 1)
        result = rt.assign_swizzle(result, "r", rt.component_wise("mix", rt.swizzle(result, "r"), rt.f(1.0), rt.binary("*", foliage, rt.f(0.7), 1), width=1))
        return rt.component_wise("clamp", result, rt.f(0.0), rt.f(1.0), width=3)
    def lutTechnicolor__vec3(rgb):
        rgb = rt.copy(rgb)
        rgb = rt.assign_swizzle(rgb, "r", rt.binary("*", rt.component_wise("pow", rt.swizzle(rgb, "r"), rt.f(0.85), width=1), rt.f(1.1), 1))
        rgb = rt.assign_swizzle(rgb, "g", rt.binary("*", rt.component_wise("pow", rt.swizzle(rgb, "g"), rt.f(1.0), width=1), rt.f(0.95), 1))
        rgb = rt.assign_swizzle(rgb, "b", rt.binary("*", rt.component_wise("pow", rt.swizzle(rgb, "b"), rt.f(0.9), width=1), rt.f(1.05), 1))
        hsl = rgbToHsl__vec3(rgb)
        hsl = rt.assign_swizzle(hsl, "y", rt.component_wise("min", rt.binary("*", rt.swizzle(hsl, "y"), rt.f(1.4), 1), rt.f(1.0), width=1))
        rgb = hslToRgb__vec3(hsl)
        rgb = rt.binary("+", rt.binary("*", rt.binary("-", rgb, rt.f(0.5), 3), rt.f(1.15), 3), rt.f(0.5), 3)
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutNeon__vec3(rgb):
        rgb = rt.copy(rgb)
        hsl = rgbToHsl__vec3(rgb)
        hsl = rt.assign_swizzle(hsl, "x", rt.component_wise("mod", rt.binary("+", rt.swizzle(hsl, "x"), rt.f(0.05), 1), rt.f(1.0), width=1))
        hsl = rt.assign_swizzle(hsl, "y", rt.component_wise("min", rt.binary("*", rt.swizzle(hsl, "y"), rt.f(1.8), 1), rt.f(1.0), width=1))
        rgb = hslToRgb__vec3(hsl)
        rgb = rt.binary("+", rt.binary("*", rt.binary("-", rgb, rt.f(0.5), 3), rt.f(1.4), 3), rt.f(0.5), 3)
    ...
    def lutMatrix__vec3(rgb):
        ...
        result = rt.construct(3, rt.binary("*", boosted, rt.f(0.2), 1), boosted, rt.binary("*", boosted, rt.f(0.15), 1))
        result = rt.binary("+", result, rt.construct(3, rt.f(0.0), rt.f(0.02), rt.f(0.0)), 3)
        return rt.component_wise("clamp", result, rt.f(0.0), rt.f(1.0), width=3)
    def lutUnderwater__vec3(rgb):
        rgb = rt.copy(rgb)
        rgb = rt.assign_swizzle(rgb, "r", rt.binary("*", rt.swizzle(rgb, "r"), rt.f(0.5), 1))
        rgb = rt.assign_swizzle(rgb, "g", rt.binary("*", rt.component_wise("pow", rt.swizzle(rgb, "g"), rt.f(0.9), width=1), rt.f(0.9), 1))
        rgb = rt.assign_swizzle(rgb, "b", rt.binary("*", rt.component_wise("pow", rt.swizzle(rgb, "b"), rt.f(0.85), width=1), rt.f(1.1), 1))
        depth = rt.binary("-", rt.f(1.0), rt.binary("*", luma__vec3(rgb), rt.f(0.3), 1), 1)
        rgb = rt.component_wise("mix", rgb, rt.binary("*", rgb, rt.construct(3, rt.f(0.4), rt.f(0.7), rt.f(1.0)), 3), rt.binary("*", rt.f(0.3), depth, 1), width=3)
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutSunset__vec3(rgb):
        ...
        rgb = rt.component_wise("mix", rt.binary("*", rgb, sunset, 3), rgb, rt.f(0.4), width=3)
        rgb = rt.assign_swizzle(rgb, "r", rt.component_wise("pow", rt.swizzle(rgb, "r"), rt.f(0.9), width=1))
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutMonochrome__vec3(rgb):
        rgb = rt.copy(rgb)
        l = luma__vec3(rgb)
        l = rt.binary("+", rt.binary("*", rt.binary("-", l, rt.f(0.5), 1), rt.f(1.2), 1), rt.f(0.5), 1)
        return rt.component_wise("clamp", rt.construct(3, l), rt.f(0.0), rt.f(1.0), width=3)
    def lutPsychedelic__vec3(rgb):
        rgb = rt.copy(rgb)
        hsl = rgbToHsl__vec3(rgb)
        hsl = rt.assign_swizzle(hsl, "x", rt.component_wise("mod", rt.binary("+", rt.binary("*", rt.swizzle(hsl, "x"), rt.f(3.0), 1), rt.binary("*", rt.swizzle(hsl, "z"), rt.f(0.5), 1), 1), rt.f(1.0), width=1))
        hsl = rt.assign_swizzle(hsl, "y", rt.component_wise("min", rt.binary("*", rt.swizzle(hsl, "y"), rt.f(2.0), 1), rt.f(1.0), width=1))
        hsl = rt.assign_swizzle(hsl, "z", rt.binary("+", rt.binary("*", rt.binary("-", rt.swizzle(hsl, "z"), rt.f(0.5), 1), rt.f(1.3), 1), rt.f(0.5), 1))
        rgb = hslToRgb__vec3(hsl)
        return rt.component_wise("clamp", rgb, rt.f(0.0), rt.f(1.0), width=3)
    def lutHardLight__vec3(rgb):
        ...
        for _for2 in range(1048576):
            ...
            if rt.binary("<", rgb[int(i)], rt.f(0.5)):
                result[int(i)] = rt.binary("*", rt.binary("*", rt.f(2.0), rgb[int(i)], 1), l, 1)
            else:
                result[int(i)] = rt.binary("-", rt.f(1.0), rt.binary("*", rt.binary("*", rt.f(2.0), rt.binary("-", rt.f(1.0), rgb[int(i)], 1), 1), rt.binary("-", rt.f(1.0), l, 1), 1), 1)
        result = rt.binary("+", rt.binary("*", rt.binary("-", result, rt.f(0.5), 3), rt.f(1.4), 3), rt.f(0.5), 3)
        highlightMask = rt.component_wise("smoothstep", rt.f(0.5), rt.f(1.0), l, width=1)
        result = rt.assign_swizzle(result, "b", rt.binary("+", rt.swizzle(result, "b"), rt.binary("*", highlightMask, rt.f(0.05), 1), 1))
        return rt.component_wise("clamp", result, rt.f(0.0), rt.f(1.0), width=3)
    def lutPosterize__vec3(rgb):
        rgb = rt.copy(rgb)
        l = luma__vec3(rgb)
        levels = rt.f(6.0)
        quantized = rt.binary("/", rt.component_wise("floor", rt.binary("+", rt.binary("*", l, levels, 1), rt.f(0.5), 1), width=1), levels, 1)
    ...
    def lutSolarize__vec3(rgb):
        ...
        for _for3 in range(1048576):
            ...
            if rt.binary(">", rgb[int(i)], threshold):
                result[int(i)] = rt.binary("*", rt.f(2.0), rt.binary("-", rt.f(1.0), rgb[int(i)], 1), 1)
            else:
                result[int(i)] = rt.binary("*", rt.f(2.0), rgb[int(i)], 1)
        hsl = rgbToHsl__vec3(result)
        hsl = rt.assign_swizzle(hsl, "y", rt.component_wise("min", rt.binary("*", rt.swizzle(hsl, "y"), rt.f(1.5), 1), rt.f(1.0), width=1))
        result = hslToRgb__vec3(hsl)
        result = rt.binary("+", rt.binary("*", rt.binary("-", result, rt.f(0.5), 3), rt.f(1.1), 3), rt.f(0.5), 3)
        return rt.component_wise("clamp", result, rt.f(0.0), rt.f(1.0), width=3)
    def main__void():
        globalCoord = rt.binary("+", rt.swizzle(ctx.frag_coord, "xy"), _u_tileOffset, 2)
        coord = cpu_ivec2__vec2(rt.swizzle(ctx.frag_coord, "xy"))
        color = rt.texel_fetch(_u_inputTex, coord, rt.i(0))
        if rt.binary("||", rt.binary("==", _u_preset, rt.i(0)), rt.binary("<=", _u_alpha, rt.f(0.0))):
            g.fragColor = color
            return
