def run_pixel(ctx, out):
    ...
    def computeVignetteMask__vec2_vec2(uv, dims):
        uv = rt.copy(uv)
        dims = rt.copy(dims)
        if rt.binary("||", rt.binary("<=", rt.swizzle(dims, "x"), rt.f(0.0)), rt.binary("<=", rt.swizzle(dims, "y"), rt.f(0.0))):
            return rt.f(0.0)
        delta = rt.component_wise("abs", rt.binary("-", uv, rt.construct(2, rt.f(0.5)), 2), width=2)
        aspect = rt.binary("/", rt.swizzle(dims, "x"), rt.component_wise("max", rt.swizzle(dims, "y"), rt.f(1.0), width=1), 1)
        scaled = rt.construct(2, rt.binary("*", rt.swizzle(delta, "x"), aspect, 1), rt.swizzle(delta, "y"))
        maxRadius = rt.length(rt.construct(2, rt.binary("*", aspect, rt.f(0.5), 1), rt.f(0.5)))
        if rt.binary("<=", maxRadius, rt.f(0.0)):
            return rt.f(0.0)
        normalizedDist = rt.component_wise("clamp", rt.binary("/", rt.length(scaled), maxRadius, 1), rt.f(0.0), rt.f(1.0), width=1)
        return rt.binary("*", normalizedDist, normalizedDist, 1)
    def main__void():
        ...
        globalUV = rt.binary("/", rt.binary("+", rt.swizzle(ctx.frag_coord, "xy"), _u_tileOffset, 2), dims, 2)
