def run_pixel(ctx, out):
    ...
    def convolve__vec2_float_bool(uv, kernel, divide):
        uv = rt.copy(uv)
        localUV = rt.binary("/", rt.binary("-", rt.binary("*", uv, _u_fullResolution, 2), _u_tileOffset, 2), rt.construct(2, rt.texture_size(_u_inputTex)), 2)
        steps = rt.binary("/", rt.f(1.0), rt.construct(2, rt.texture_size(_u_inputTex)), 2)
        ...
        for _for0 in range(1048576):
            ...
            if not (rt.binary("<", i, rt.i(9))):
                break
            color = rt.swizzle(rt.texture(_u_inputTex, rt.binary("+", localUV, rt.binary("*", offset[int(i)], rt.component_wise("floor", map__float_float_float_float_float(_u_amount, rt.f(0.0), rt.f(100.0), rt.f(0.0), rt.f(20.0)), width=1), 1), 2)), "rgb")
            conv = rt.binary("+", conv, rt.binary("*", color, kernel[int(i)], 3), 3)
            kernelWeight = rt.binary("+", kernelWeight, kernel[int(i)], 1)
        if divide:
            conv = rt.assign_swizzle(conv, "rgb", rt.binary("/", rt.swizzle(conv, "rgb"), kernelWeight, 3))
        return rt.component_wise("clamp", rt.swizzle(conv, "rgb"), rt.f(0.0), rt.f(1.0), width=3)
    def desaturate__vec3(color):
        color = rt.copy(color)
        return rt.binary("+", rt.binary("+", rt.binary("*", rt.f(0.2126), rt.swizzle(color, "r"), 1), rt.binary("*", rt.f(0.7152), rt.swizzle(color, "g"), 1), 1), rt.binary("*", rt.f(0.0722), rt.swizzle(color, "b"), 1), 1)
    ...
    def main__void():
        ...
        brightness = rt.binary("+", desaturate__vec3(rt.swizzle(inputColor, "rgb")), rt.binary("/", _u_direction, rt.f(360.0), 1), 1)
        displacement = rt.binary("*", _u_amount, rt.f(0.01), 1)
        if rt.binary("||", rt.binary(">", rt.swizzle(_u_fullResolution, "x"), rt.swizzle(_u_resolution, "x")), rt.binary(">", rt.swizzle(_u_fullResolution, "y"), rt.swizzle(_u_resolution, "y"))):
            maxDisplacement = rt.binary("/", rt.f(256.0), rt.component_wise("max", rt.swizzle(_u_fullResolution, "x"), rt.swizzle(_u_fullResolution, "y"), width=1), 1)
            displacement = rt.component_wise("min", displacement, maxDisplacement, width=1)
        if rt.binary("==", _u_mode, rt.i(0)):
            uv = rt.assign_swizzle(uv, "x", rt.binary("+", rt.swizzle(uv, "x"), rt.binary("*", rt.component_wise("cos", rt.binary("*", brightness, rt.f(6.28318530718), 1), width=1), displacement, 1), 1))
            uv = rt.assign_swizzle(uv, "y", rt.binary("+", rt.swizzle(uv, "y"), rt.binary("*", rt.component_wise("sin", rt.binary("*", brightness, rt.f(6.28318530718), 1), width=1), displacement, 1), 1))
        else:
            if rt.binary("==", _u_mode, rt.i(1)):
                uv = rt.assign_swizzle(uv, "y", rt.binary("+", rt.swizzle(uv, "y"), rt.binary("*", desaturate__vec3(derivX__vec3_vec2_bool(rt.swizzle(inputColor, "rgb"), uv, False)), displacement, 1), 1))
                uv = rt.assign_swizzle(uv, "x", rt.binary("+", rt.swizzle(uv, "x"), rt.binary("*", desaturate__vec3(derivY__vec3_vec2_bool(rt.swizzle(inputColor, "rgb"), uv, False)), displacement, 1), 1))
        ...
        warpedLocalUV = rt.binary("/", rt.binary("-", rt.binary("*", uv, _u_fullResolution, 2), _u_tileOffset, 2), rt.construct(2, rt.texture_size(_u_inputTex)), 2)
