def run_pixel(ctx, out):
    ...
    def concentricRings__vec2_float_float(p, t, timeOffset):
        p = rt.copy(p)
        d = rt.component_wise("fract", rt.binary("+", rt.length(p), timeOffset, 1), width=1)
    ...
    def radialLines__vec2_float_float(p, t, timeOffset):
        ...
        angle = rt.binary("+", rt.component_wise("atan", rt.swizzle(p, "y"), rt.swizzle(p, "x"), width=1), rt.binary("*", timeOffset, rt.f(6.28318530718), 1), 1)
    ...
    def heartSDF__vec2(p):
        ...
        if rt.binary(">", rt.binary("+", rt.swizzle(p, "y"), rt.swizzle(p, "x"), 1), rt.f(1.0)):
            return rt.binary("-", rt.component_wise("sqrt", rt.dot(rt.binary("-", p, rt.construct(2, rt.f(0.25), rt.f(0.75)), 2), rt.binary("-", p, rt.construct(2, rt.f(0.25), rt.f(0.75)), 2)), width=1), rt.binary("/", rt.component_wise("sqrt", rt.f(2.0), width=1), rt.f(4.0), 1), 1)
        return rt.binary("*", rt.component_wise("sqrt", rt.component_wise("min", rt.dot(rt.binary("-", p, rt.construct(2, rt.f(0.0), rt.f(1.0)), 2), rt.binary("-", p, rt.construct(2, rt.f(0.0), rt.f(1.0)), 2)), rt.dot(rt.binary("-", p, rt.binary("*", rt.f(0.5), rt.component_wise("max", rt.binary("+", rt.swizzle(p, "x"), rt.swizzle(p, "y"), 1), rt.f(0.0), width=1), 1), 2), rt.binary("-", p, rt.binary("*", rt.f(0.5), rt.component_wise("max", rt.binary("+", rt.swizzle(p, "x"), rt.swizzle(p, "y"), 1), rt.f(0.0), width=1), 1), 2)), width=1), width=1), rt.component_wise("sign", rt.binary("-", rt.swizzle(p, "x"), rt.swizzle(p, "y"), 1), width=1), 1)
    def hearts__vec2_float(p, t):
        p = rt.copy(p)
        cell = rt.binary("-", rt.component_wise("fract", p, width=2), rt.f(0.5), 2)
        cell = rt.assign_swizzle(cell, "y", rt.binary("+", rt.swizzle(cell, "y"), rt.f(0.25), 1))
        d = heartSDF__vec2(rt.binary("*", cell, rt.f(2.4), 2))
        radius = rt.binary("-", rt.f(0.15), rt.binary("*", t, rt.f(0.15), 1), 1)
        sm = rt.component_wise("min", _u_smoothness, rt.binary("+", radius, rt.f(0.15), 1), width=1)
        return rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.binary("-", rt.unary("-", radius), sm, 1), rt.binary("+", rt.unary("-", radius), sm, 1), d, width=1), 1)
    def waves__vec2_float(p, t):
        p = rt.copy(p)
        y = rt.binary("-", rt.component_wise("fract", rt.swizzle(p, "y"), width=1), rt.f(0.5), 1)
        y = rt.binary("-", y, rt.binary("*", rt.component_wise("cos", rt.binary("*", rt.swizzle(p, "x"), rt.f(6.28318530718), 1), width=1), rt.f(0.15), 1), 1)
        dist = rt.component_wise("abs", y, width=1)
        halfW = rt.binary("*", t, rt.f(0.2), 1)
        sm = rt.component_wise("min", _u_smoothness, rt.binary("+", halfW, rt.f(0.01), 1), width=1)
        return rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.binary("-", halfW, sm, 1), rt.binary("+", halfW, sm, 1), dist, width=1), 1)
    def zigzag__vec2_float(p, t):
        p = rt.copy(p)
        f = rt.component_wise("fract", p, width=2)
        lineY = rt.binary("-", rt.f(1.0), rt.binary("*", rt.f(2.0), rt.component_wise("abs", rt.binary("-", rt.swizzle(f, "x"), rt.f(0.5), 1), width=1), 1), 1)
        dist = rt.component_wise("abs", rt.binary("-", rt.binary("-", rt.swizzle(f, "y"), rt.binary("*", lineY, rt.f(0.5), 1), 1), rt.f(0.25), 1), width=1)
        halfW = rt.binary("*", t, rt.f(0.12), 1)
        sm = rt.component_wise("min", _u_smoothness, rt.component_wise("max", rt.binary("-", rt.f(0.24), halfW, 1), rt.f(0.005), width=1), width=1)
        return rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.binary("-", halfW, sm, 1), rt.binary("+", halfW, sm, 1), dist, width=1), 1)
    def main__void():
        ...
        centered = rt.binary("||", rt.binary("==", _u_patternType, rt.i(1)), rt.binary("||", rt.binary("==", _u_patternType, rt.i(5)), rt.binary("==", _u_patternType, rt.i(6))))
        if rt.binary("&&", rt.unary("!", centered), rt.binary("==", _u_animation, rt.i(2))):
            st = rotate2D__vec2_float(st, rt.binary("*", rt.binary("*", _u_time, rt.f(6.28318530718), 1), rt.component_wise("floor", _u_speed, width=1), 1))
        st = rt.assign_swizzle(st, "x", rt.binary("+", rt.swizzle(st, "x"), rt.binary("*", rt.swizzle(st, "y"), _u_skew, 1), 1))
        p = rt.binary("*", st, rt.binary("-", rt.f(21.0), _u_scale, 1), 2)
        if rt.binary("&&", rt.unary("!", centered), rt.binary("==", _u_animation, rt.i(1))):
            panPeriod = (rt.f(2.0) if rt.binary("==", _u_patternType, rt.i(0)) else rt.f(1.0))
            p = rt.assign_swizzle(p, "x", rt.binary("+", rt.swizzle(p, "x"), rt.binary("*", rt.binary("*", _u_time, rt.unary("-", rt.component_wise("floor", _u_speed, width=1)), 1), panPeriod, 1), 1))
        m = rt.f(0.0)
        if rt.binary("==", _u_patternType, rt.i(0)):
            m = checkerboard__vec2_float(p, _u_smoothness)
        else:
            if rt.binary("==", _u_patternType, rt.i(1)):
                m = concentricRings__vec2_float_float(p, _u_thickness, rt.binary("*", rt.unary("-", _u_time), rt.component_wise("floor", _u_speed, width=1), 1))
            else:
                    ...
                    if rt.binary("==", _u_patternType, rt.i(3)):
                        ...
                        if rt.binary("==", _u_patternType, rt.i(4)):
                            ...
                            if rt.binary("==", _u_patternType, rt.i(5)):
                                m = radialLines__vec2_float_float(p, _u_thickness, rt.binary("*", _u_time, rt.component_wise("floor", _u_speed, width=1), 1))
                            else:
                                if rt.binary("==", _u_patternType, rt.i(6)):
                                    m = spiralPattern__vec2_float_float(p, _u_thickness, rt.binary("*", rt.unary("-", _u_time), rt.component_wise("floor", _u_speed, width=1), 1))
                                else:
