def run_pixel(ctx, out):
    ...
    def linear2srgb__vec3(linear):
        ...
        high = rt.binary("-", rt.binary("*", rt.f(1.055), rt.component_wise("pow", rt.component_wise("max", linear, rt.construct(3, rt.f(0.0)), width=3), rt.construct(3, rt.binary("/", rt.f(1.0), rt.f(2.4), 1)), width=3), 3), rt.f(0.055), 3)
    ...
    def oklab2rgb__vec3(lab):
        lab = rt.copy(lab)
        L = rt.swizzle(lab, "x")
        a = rt.binary("*", rt.binary("-", rt.swizzle(lab, "y"), rt.f(0.5), 1), rt.f(0.8), 1)
        b = rt.binary("*", rt.binary("-", rt.swizzle(lab, "z"), rt.f(0.5), 1), rt.f(0.8), 1)
    ...
    def oklch2rgb__vec3(lch):
        lch = rt.copy(lch)
        L = rt.swizzle(lch, "x")
        C = rt.binary("*", rt.swizzle(lch, "y"), rt.f(0.4), 1)
        H = rt.binary("*", rt.swizzle(lch, "z"), g.TAU, 1)
    ...
    def main__void():
        ...
        t = rt.binary("+", rt.binary("*", lum, _u_repeat, 1), _u_offset, 1)
        ...
        _local_offset_1 = rt.construct(3, _u_offsetR, _u_offsetG, _u_offsetB)
        ...
        paletteColor = cosinePalette__float_vec3_vec3_vec3_vec3(t, _local_offset_1, amp, freq, phase)
