        ...
        for _for0 in range(1048576):
            ...
            for _for1 in range(1048576):
                ...
                if not (rt.binary("<=", dx, rt.i(1))):
                    break
                neighbor = rt.construct(2, dx, dy)
                cellId = rt.binary("+", cell, neighbor, 2)
                k = rt.i(0)
                _for2_first = True
                for _for2 in range(1048576):
                    ...
                    if rt.binary(">=", k, impulses):
                        break
                    r1 = prng__vec3(rt.construct(3, cellId, rt.binary("+", sd, rt.binary("*", k, rt.f(7.0), 1), 1)))
                    r2 = prng__vec3(rt.construct(3, rt.binary("+", sd, rt.binary("*", k, rt.f(13.0), 1), 1), cellId))
                    impulsePos = rt.swizzle(r1, "xy")
                    impulsePos = rt.binary("+", impulsePos, rt.binary("*", rt.construct(2, rt.component_wise("sin", rt.binary("+", t, rt.binary("*", rt.swizzle(r2, "x"), rt.f(6.28318530718), 1), 1), width=1), rt.component_wise("cos", rt.binary("+", t, rt.binary("*", rt.swizzle(r2, "y"), rt.f(6.28318530718), 1), 1), width=1)), rt.f(0.15), 2), 2)
                    delta = rt.binary("-", rt.binary("+", neighbor, impulsePos, 2), frac, 2)
                    angle = rt.component_wise("mix", baseAngle, rt.binary("*", rt.swizzle(r2, "z"), rt.f(6.28318530718), 1), iso, width=1)
                    ...
                    envelope = rt.component_wise("exp", rt.binary("/", rt.unary("-", rt.dot(delta, delta)), rt.binary("*", rt.binary("*", rt.f(2.0), sigma, 1), sigma, 1), 1), width=1)
                    phase = rt.binary("*", rt.binary("*", rt.f(6.28318530718), freq, 1), rt.dot(dir, delta), 1)
                    sum = rt.binary("+", sum, rt.binary("*", rt.binary("*", weight, envelope, 1), rt.component_wise("cos", phase, width=1), 1), 1)
        return sum
    def main__void():
        ...
        baseAngle = rt.binary("/", rt.binary("*", _u_orientation, rt.f(3.14159265359), 1), rt.f(180.0), 1)
        iso = rt.binary("/", _u_isotropy, rt.f(100.0), 1)
        impulses = rt.construct(1, _u_density)
        oct = rt.construct(1, _u_octaves)
        spd = rt.component_wise("floor", _u_speed, width=1)
        t = rt.binary("*", rt.binary("*", _u_time, rt.f(6.28318530718), 1), spd, 1)
        p = rt.binary("*", st, freq, 2)
        value = rt.f(0.0)
        amplitude = rt.f(1.0)
        totalAmp = rt.f(0.0)
        pOct = p
        ...
        for _for3 in range(1048576):
            ...
            if rt.binary(">=", i, oct):
                break
            octFreq = rt.binary("+", rt.f(1.0), rt.binary("*", i, rt.f(0.5), 1), 1)
            octSigma = rt.binary("/", sigma, rt.binary("+", rt.f(1.0), rt.binary("*", i, rt.f(0.5), 1), 1), 1)
            fi = i
            value = rt.binary("+", value, rt.binary("*", amplitude, gaborNoise__vec2_float_float_float_float_int_float_float(pOct, octFreq, octSigma, baseAngle, iso, impulses, rt.binary("+", t, rt.binary("*", fi, rt.f(3.7), 1), 1), rt.binary("+", _u_seed, rt.binary("*", fi, rt.f(17.0), 1), 1)), 1), 1)
            totalAmp = rt.binary("+", totalAmp, amplitude, 1)
            amplitude = rt.binary("*", amplitude, rt.f(0.5), 1)
            pOct = rt.binary("*", pOct, rt.f(2.0), 2)
        value = rt.binary("/", value, totalAmp, 1)
        n = rt.binary("/", rt.f(1.0), rt.binary("+", rt.f(1.0), rt.component_wise("exp", rt.binary("*", rt.unary("-", value), rt.f(3.0), 1), width=1), 1), 1)
