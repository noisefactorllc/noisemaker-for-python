def run_pixel(ctx, out):
    ...
    def transformCoords_df64__vec2_vec2_vec2_float_float_vec2_vec2(fragCoord, cX_df, cY_df, z_zoom, rot, re_df, im_df):
        ...
        scale = rt.binary("/", rt.f(2.5), z_zoom, 1)
    ...
    def getPOI__int(idx):
        if rt.binary("==", idx, rt.i(1)):
            return rt.component_wise("POIData", rt.construct(4, rt.f(0.0), rt.f(0.0), rt.f(0.0), rt.f(0.0)), rt.f(3.0), rt.f(7.0), width=4)
        if rt.binary("==", idx, rt.i(2)):
            return rt.component_wise("POIData", rt.construct(4, rt.f(0.25), rt.f(0.4330126941204071), rt.f(0.0), rt.f(7.7718e-9)), rt.f(3.0), rt.f(14.0), width=4)
        if rt.binary("==", idx, rt.i(3)):
            return rt.component_wise("POIData", rt.construct(4, rt.f(0.0), rt.f(0.0), rt.f(0.0), rt.f(0.0)), rt.f(5.0), rt.f(7.0), width=4)
        if rt.binary("==", idx, rt.i(4)):
            return rt.component_wise("POIData", rt.construct(4, rt.f(0.6545084714889526), rt.f(0.4755282700061798), rt.f(2.5699e-8), rt.unary("-", rt.f(1.1859e-8))), rt.f(5.0), rt.f(14.0), width=4)
        if rt.binary("==", idx, rt.i(5)):
            return rt.component_wise("POIData", rt.construct(4, rt.f(0.0), rt.f(0.0), rt.f(0.0), rt.f(0.0)), rt.f(6.0), rt.f(7.0), width=4)
        if rt.binary("==", idx, rt.i(6)):
            return rt.component_wise("POIData", rt.construct(4, rt.f(0.0), rt.f(0.0), rt.f(0.0), rt.f(0.0)), rt.f(8.0), rt.f(7.0), width=4)
        return rt.component_wise("POIData", rt.construct(4, rt.f(0.0)), rt.f(3.0), rt.f(7.0), width=4)
    def main__void():
        globalCoord = rt.binary("+", rt.swizzle(ctx.frag_coord, "xy"), _u_tileOffset, 2)
        maxIter = rt.construct(1, _u_iterations)
        poiIdx = rt.construct(1, _u_poi)
        outMode = rt.construct(1, _u_outputMode)
        doInvert = rt.binary(">", _u_invert, rt.f(0.5))
        effDegree = _u_degree
        if rt.binary("&&", rt.binary(">", _u_degreeSpeed, rt.f(0.0)), rt.binary(">", _u_degreeRange, rt.f(0.0))):
            effDegree = rt.binary("+", effDegree, rt.binary("*", _u_degreeRange, rt.component_wise("sin", rt.binary("*", rt.binary("*", _u_time, _u_degreeSpeed, 1), g.TAU, 1), width=1), 1), 1)
            effDegree = rt.component_wise("clamp", effDegree, rt.f(3.0), rt.f(8.0), width=1)
        effRelax = _u_relaxation
        if rt.binary("&&", rt.binary(">", _u_relaxSpeed, rt.f(0.0)), rt.binary(">", _u_relaxRange, rt.f(0.0))):
            effRelax = rt.binary("+", effRelax, rt.binary("*", _u_relaxRange, rt.component_wise("sin", rt.binary("*", rt.binary("*", rt.binary("*", _u_time, _u_relaxSpeed, 1), g.TAU, 1), g.PHI, 1), width=1), 1), 1)
        ...
        if rt.binary(">", poiIdx, rt.i(0)):
            ...
            cHi = rt.binary("+", rt.swizzle(rt.swizzle(p, "center"), "xy"), rt.construct(2, _u_centerHiX, _u_centerHiY), 2)
            cLo = rt.binary("+", rt.swizzle(rt.swizzle(p, "center"), "zw"), rt.construct(2, _u_centerLoX, _u_centerLoY), 2)
            effDegree = rt.swizzle(p, "deg")
            effZoomDepth = rt.component_wise("min", _u_zoomDepth, rt.swizzle(p, "maxZoom"), width=7)
        else:
        ...
        if rt.binary(">", _u_zoomSpeed, rt.f(0.0)):
            ...
            zoom = rt.component_wise("pow", rt.f(10.0), rt.binary("*", effZoomDepth, zoomPhase, 1), width=1)
        else:
        ...
        intDeg = rt.construct(1, rt.component_wise("floor", effDegree, width=1))
        numRoots = intDeg
        roots = rt.construct(2, 0.0)
        k = rt.i(0)
        _for0_first = True
        for _for0 in range(1048576):
            ...
            if rt.binary(">=", k, numRoots):
                break
            angle = rt.binary("/", rt.binary("*", g.TAU, k, 1), intDeg, 1)
        ...
        bailout = rt.binary("*", rt.f(1e10), effRelax, 1)
        zr_df = re_df
        zi_df = im_df
        n = rt.i(0)
        _for1_first = True
        for _for1 in range(1048576):
            ...
            for _for2 in range(1048576):
                ...
                if not (rt.binary("<", j, rt.i(7))):
                    break
                if rt.binary(">=", j, rt.binary("-", intDeg, rt.i(1), 1)):
                    break
            ...
            fzi = zni
            fpzr = df64_mul_f__vec2_float(pwr, intDeg)
            fpzi = df64_mul_f__vec2_float(pwi, intDeg)
            fpzr_f = df64_to_float__vec2(fpzr)
            fpzi_f = df64_to_float__vec2(fpzi)
            if rt.binary("<", rt.binary("+", rt.binary("*", fpzr_f, fpzr_f, 1), rt.binary("*", fpzi_f, fpzi_f, 1), 1), rt.f(1e-20)):
                break
            denom = rt.binary("+", rt.binary("*", fpzr_f, fpzr_f, 1), rt.binary("*", fpzi_f, fpzi_f, 1), 1)
            inv_denom = rt.binary("/", rt.f(1.0), denom, 1)
            ...
            if rt.binary(">", rt.binary("+", rt.binary("*", zx, zx, 1), rt.binary("*", zy, zy, 1), 1), bailout):
                break
            k = rt.i(0)
            _for3_first = True
            for _for3 in range(1048576):
                ...
                if rt.binary(">=", k, numRoots):
                    break
                dx = rt.binary("-", zx, rt.swizzle(roots[int(k)], "x"), 1)
                dy = rt.binary("-", zy, rt.swizzle(roots[int(k)], "y"), 1)
                d = rt.component_wise("sqrt", rt.binary("+", rt.binary("*", dx, dx, 1), rt.binary("*", dy, dy, 1), 1), width=1)
            ...
            if rt.binary(">=", convergedRoot, rt.i(0)):
                break
            iter = rt.binary("+", iter, rt.f(1.0), 1)
        smoothIter = iter
        if rt.binary("&&", rt.binary(">=", convergedRoot, rt.i(0)), rt.binary("&&", rt.binary(">", convergeDist, rt.f(0.0)), rt.binary("<", convergeDist, _u_tolerance))):
            smoothIter = rt.binary("-", iter, rt.component_wise("log2", rt.binary("/", rt.component_wise("log", convergeDist, width=1), rt.component_wise("log", _u_tolerance, width=1), 1), width=1), 1)
        value = rt.f(0.0)
        maxIterF = maxIter
        numRootsF = numRoots
        if rt.binary("==", outMode, rt.i(0)):
            value = rt.binary("/", smoothIter, maxIterF, 1)
        else:
            if rt.binary("==", outMode, rt.i(1)):
                if rt.binary(">=", convergedRoot, rt.i(0)):
                    value = rt.binary("/", convergedRoot, numRootsF, 1)
            else:
                if rt.binary(">=", convergedRoot, rt.i(0)):
                    value = rt.binary("/", rt.binary("+", convergedRoot, rt.binary("/", smoothIter, maxIterF, 1), 1), numRootsF, 1)
        if doInvert:
