def run_pixel(ctx, out):
    ...
    def colorGeneralEmboss__vec2_vec2(uv, texelSize):
        ...
        kernel[int(rt.i(7))] = rt.f(1.0)
        kernel[int(rt.i(8))] = rt.f(2.0)
        baseOffsetsPx = rt.construct(2, 0.0)
        ...
        theta = rt.component_wise("radians", rt.binary("-", _u_angle, rt.f(135.0), 1), width=1)
        ...
        for _for1 in range(1048576):
            ...
            rotatedPx = rt.binary("*", rt.construct(2, rt.binary("+", rt.binary("*", ct, rt.swizzle(basePx, "x"), 1), rt.binary("*", st, rt.swizzle(basePx, "y"), 1), 1), rt.binary("+", rt.binary("*", rt.unary("-", st), rt.swizzle(basePx, "x"), 1), rt.binary("*", ct, rt.swizzle(basePx, "y"), 1), 1)), _u_height, 2)
            offsetUV = rt.binary("*", rt.binary("*", rt.binary("*", rotatedPx, texelSize, 2), _u_amount, 2), _u_renderScale, 2)
            texSample = rt.swizzle(rt.texture(_u_inputTex, rt.binary("/", rt.binary("-", rt.binary("*", rt.binary("+", uv, offsetUV, 2), _u_fullResolution, 2), _u_tileOffset, 2), rt.construct(2, rt.texture_size(_u_inputTex)), 2)), "rgb")
            conv = rt.binary("+", conv, rt.binary("*", texSample, kernel[int(i)], 3), 3)
        return conv
    def grayEmboss__vec2_vec3(uv, centerRGB):
        ...
        offsetUV = rt.binary("/", rt.binary("*", direction, rt.binary("*", _u_height, _u_renderScale, 1), 2), _u_fullResolution, 2)
        positiveLuma = rt.dot(sampleGlobal__vec2(rt.binary("+", uv, offsetUV, 2)), g.LUMA)
        negativeLuma = rt.dot(sampleGlobal__vec2(rt.binary("-", uv, offsetUV, 2)), g.LUMA)
        signedEdge = rt.binary("-", positiveLuma, negativeLuma, 1)
        edgeMagnitude = rt.component_wise("abs", signedEdge, width=1)
        relief = rt.binary("+", rt.f(0.5), rt.binary("*", rt.f(0.5), signedEdge, 1), 1)
        centerLuma = rt.dot(centerRGB, g.LUMA)
        sourceChroma = rt.binary("-", centerRGB, rt.construct(3, centerLuma), 3)
        tracedColor = rt.binary("*", rt.binary("*", sourceChroma, edgeMagnitude, 3), rt.component_wise("clamp", rt.binary("/", _u_colorAmount, rt.f(100.0), 1), rt.f(0.0), rt.f(1.0), width=1), 3)
        return rt.binary("+", rt.construct(3, relief), tracedColor, 3)
    def main__void():
        ...
        origColor = rt.texture(_u_inputTex, rt.binary("/", rt.swizzle(ctx.frag_coord, "xy"), resolution, 2))
        fullFrame = rt.binary("&&", rt.component_wise("all", rt.component_wise("equal", _u_tileOffset, rt.construct(2, rt.f(0.0)), width=2), width=2), rt.component_wise("all", rt.component_wise("equal", _u_fullResolution, resolution, width=2), width=2))
        colorTexelSize = (texelSize if fullFrame else rt.binary("/", rt.f(1.0), _u_fullResolution, 2))
        result = rt.construct(3, 0.0)
        if rt.binary("==", _u_STYLE, rt.i(0)):
            if rt.binary("&&", rt.binary("==", _u_angle, rt.f(135.0)), rt.binary("==", _u_height, rt.f(1.0))):
