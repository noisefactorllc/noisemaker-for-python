def run_pixel(ctx, out):
    ...
    _u_sharpenRadius = U.get("sharpenRadius", 0)
    _u_sharpenAmount = U.get("sharpenAmount", rt.f(0.0))
    ...
    def main__void():
        ...
        scaledRadius = rt.construct(1, rt.binary("*", rt.construct(1, _u_sharpenRadius), _u_renderScale, 1, "float"), base="int")
        if (bool(rt.binary("<=", scaledRadius, rt.i(0))) or bool(rt.binary("<=", _u_sharpenAmount, rt.f(0.0)))):
            g.fragColor[:] = center
            return
        sigma = rt.binary("/", rt.construct(1, scaledRadius), rt.f(2.0), 1, "float")
        sigma2 = rt.binary("*", sigma, sigma, 1, "float")
        blurSum = rt.construct(3, rt.f(0.0))
        weightSum = rt.f(0.0)
        ...
        for _for0 in range(1048576):
            ...
            for _for1 in range(1048576):
                ...
                blurSum[:] = rt.binary("+", blurSum, rt.binary("*", rt.swizzle(texSample, "rgb"), weight, 3, "float"), 3, "float")
        ...
        sharpened[:] = rt.component_wise("clamp", sharpened, rt.f(0.0), rt.f(1.0), width=3)
        g.fragColor[:] = rt.construct(4, sharpened, rt.swizzle(center, "a"))
    main__void()
    _c = g.fragColor
