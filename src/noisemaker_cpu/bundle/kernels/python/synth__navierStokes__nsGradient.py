def run_pixel(ctx, out):
    ...
    def main__void():
        ...
        pR = rt.swizzle(rt.texture(_u_pressureTex, rt.binary("+", uv, rt.array([rt.swizzle(texel, "x"), rt.f(0.0)]), 2, "float")), "r")
        pL = rt.swizzle(rt.texture(_u_pressureTex, rt.binary("-", uv, rt.array([rt.swizzle(texel, "x"), rt.f(0.0)]), 2, "float")), "r")
        pT = rt.swizzle(rt.texture(_u_pressureTex, rt.binary("+", uv, rt.array([rt.f(0.0), rt.swizzle(texel, "y")]), 2, "float")), "r")
        pB = rt.swizzle(rt.texture(_u_pressureTex, rt.binary("-", uv, rt.array([rt.f(0.0), rt.swizzle(texel, "y")]), 2, "float")), "r")
