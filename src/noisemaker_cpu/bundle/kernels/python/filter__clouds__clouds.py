def run_pixel(ctx, out):
    ...
    def simplex2d__vec2(v):
        ...
        g = rt.assign_swizzle(g, "x", rt.binary("+", rt.binary("*", rt.swizzle(a0, "x"), rt.swizzle(x0, "x"), 1, "float"), rt.binary("*", rt.swizzle(h, "x"), rt.swizzle(x0, "y"), 1, "float"), 1, "float"))
        g = rt.assign_swizzle(g, "yz", rt.binary("+", rt.binary("*", rt.swizzle(a0, "yz"), rt.swizzle(x12, "xz"), 2, "float"), rt.binary("*", rt.swizzle(h, "yz"), rt.swizzle(x12, "yw"), 2, "float"), 2, "float"))
        return rt.binary("*", rt.f(130.0), rt.dot(m, g), 1, "float")
