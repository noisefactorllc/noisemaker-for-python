def run_pixel(ctx, out):
    ...
    def cloudNoise__vec2_float_int_float_float(uv, baseFreq, octaves, animPhase, animSpeed):
        ...
        for _for0 in range(1048576):
            ...
            timeOffset = rt.binary("*", rt.binary("-", rt.construct(2, rt.component_wise("cos", rt.binary("+", animPhase, octavePhase, 1, "float"), width=1), rt.component_wise("sin", rt.binary("+", animPhase, octavePhase, 1, "float"), width=1)), rt.array([rt.component_wise("cos", octavePhase, width=1), rt.component_wise("sin", octavePhase, width=1)]), 2, "float"), octaveRadius, 2, "float")
            n = simplex2d__vec2(rt.binary("+", rt.binary("+", rt.binary("*", uv, freq, 2, "float"), rt.array([rt.binary("*", rt.construct(1, i), rt.f(37.0), 1, "float"), rt.binary("*", rt.construct(1, i), rt.f(53.0), 1, "float")]), 2, "float"), timeOffset, 2, "float"))
    ...
    def main__void():
        ...
        cloudUV = rt.binary("+", rt.binary("/", rt.binary("*", globalUV, rt.array([aspect, rt.f(1.0)]), 2, "float"), _u_scale, 2, "float"), seedOffset, 2, "float")
        ...
        shadowUV = rt.binary("+", rt.binary("/", rt.binary("*", rt.binary("+", globalUV, shadowOffset, 2, "float"), rt.array([aspect, rt.f(1.0)]), 2, "float"), _u_scale, 2, "float"), seedOffset, 2, "float")
