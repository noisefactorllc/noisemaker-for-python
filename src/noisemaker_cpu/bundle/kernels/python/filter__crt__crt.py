def run_pixel(ctx, out):
    ...
    def random_scalar__float(seed):
        return rt.component_wise("fract", rt.binary("*", rt.component_wise("sin", seed, width=1), rt.f(43758.5453123), 1), width=1)
    def simplex_random__float_float(time, speed):
        angle = rt.binary("*", time, g.TAU, 1)
        z = rt.binary("*", rt.component_wise("cos", angle, width=1), speed, 1)
        w = rt.binary("*", rt.component_wise("sin", angle, width=1), speed, 1)
        return rt.component_wise("fract", rt.binary("*", rt.component_wise("sin", rt.binary("+", rt.binary("*", z, rt.f(157.0), 1), rt.binary("*", w, rt.f(113.0), 1), 1), width=1), rt.f(43758.5453), 1), width=1)
    def freq_for_shape__float_float_float(base_freq, width, height):
        ...
        if rt.binary("<", rt.component_wise("abs", rt.binary("-", width_safe, height_safe, 1), width=1), rt.f(1e-5)):
            return rt.construct(2, freq, freq)
        if rt.binary("<", height_safe, width_safe):
            scaled = rt.component_wise("floor", rt.binary("/", rt.binary("*", freq, width_safe, 1), height_safe, 1), width=1)
            return rt.construct(2, freq, rt.component_wise("max", scaled, rt.f(1.0), width=1))
        scaled = rt.component_wise("floor", rt.binary("/", rt.binary("*", freq, height_safe, 1), width_safe, 1), width=1)
    ...
    def animated_simplex_value__vec2_float_float(uv, time, speed):
        ...
        s = rt.binary("*", _u_seed, rt.f(73.0), 1)
        base_seed = rt.construct(3, rt.binary("+", rt.f(17.0), s, 1), rt.binary("+", rt.f(29.0), rt.binary("*", s, rt.f(1.1), 1), 1), rt.binary("+", rt.f(47.0), rt.binary("*", s, rt.f(0.7), 1), 1))
    ...
    def compute_lens_offsets__vec2_float_float_vec2_float_float_float(sample_pos, width, height, freq, time, speed, displacement):
        ...
        uv = rt.construct(2, rt.binary("*", rt.binary("/", rt.swizzle(wrapped_pos, "x"), width_safe, 1), freq_x, 1), rt.binary("*", rt.binary("/", rt.swizzle(wrapped_pos, "y"), height_safe, 1), freq_y, 1))
        noise_value = animated_simplex_value__vec2_float_float(uv, time, speed)
        uv_centered = rt.binary("/", rt.binary("+", wrapped_pos, rt.construct(2, rt.f(0.5), rt.f(0.5)), 2), rt.construct(2, width_safe, height_safe), 2)
        mask = singularity_mask__vec2_float_float(uv_centered, width_safe, height_safe)
        distortion = rt.binary("*", rt.binary("-", rt.binary("*", noise_value, rt.f(2.0), 1), rt.f(1.0), 1), mask, 1)
        angle = rt.binary("*", distortion, g.TAU, 1)
        offsets = rt.binary("*", rt.binary("*", rt.construct(2, rt.component_wise("cos", angle, width=1), rt.component_wise("sin", angle, width=1)), displacement, 2), rt.construct(2, width_safe, height_safe), 2)
        return offsets
    def fade__float(value):
        return rt.binary("*", rt.binary("*", value, value, 1), rt.binary("-", rt.f(3.0), rt.binary("*", rt.f(2.0), value, 1), 1), 1)
    ...
    def lerp__float_float_float(a, b, t):
        return rt.binary("+", a, rt.binary("*", rt.binary("-", b, a, 1), t, 1), 1)
    def hash3__ivec3_float(coord, seed):
        coord = rt.copy(coord)
        base = rt.construct(3, coord)
        dot_value = rt.binary("+", rt.dot(base, rt.construct(3, rt.f(12.9898), rt.f(78.233), rt.f(37.719))), rt.binary("*", seed, rt.f(0.001), 1), 1)
        return rt.component_wise("fract", rt.binary("*", rt.component_wise("sin", dot_value, width=1), rt.f(43758.5453), 1), width=1)
    def value_noise_3d__vec3_float(coord, seed):
        coord = rt.copy(coord)
        cell = rt.construct(3, rt.component_wise("floor", coord, width=3))
        cell_i = cpu_ivec3__vec3(cell)
        local = rt.component_wise("fract", coord, width=3)
        smooth_t = fade_vec3__vec3(local)
        c000 = hash3__ivec3_float(cell_i, seed)
        c100 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(1), rt.i(0), rt.i(0)), 3), seed)
        c010 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(0), rt.i(1), rt.i(0)), 3), seed)
        c110 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(1), rt.i(1), rt.i(0)), 3), seed)
        c001 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(0), rt.i(0), rt.i(1)), 3), seed)
        c101 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(1), rt.i(0), rt.i(1)), 3), seed)
        c011 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(0), rt.i(1), rt.i(1)), 3), seed)
        c111 = hash3__ivec3_float(rt.binary("+", cell_i, cpu_ivec3__float_float_float(rt.i(1), rt.i(1), rt.i(1)), 3), seed)
    ...
    def compute_singularity__float_float_float_float(x, y, width, height):
        center_x = rt.binary("*", width, rt.f(0.5), 1)
        center_y = rt.binary("*", height, rt.f(0.5), 1)
        dx = rt.binary("/", rt.binary("-", x, center_x, 1), width, 1)
        dy = rt.binary("/", rt.binary("-", y, center_y, 1), height, 1)
        return rt.length(rt.construct(2, dx, dy))
    def wrap_unit__float(value):
        wrapped = rt.binary("-", value, rt.component_wise("floor", value, width=1), 1)
        if rt.binary("<", wrapped, rt.f(0.0)):
            wrapped = rt.binary("+", wrapped, rt.f(1.0), 1)
        return wrapped
    ...
    def blend_cosine__float_float_float(a, b, value):
        ...
        weight = rt.binary("*", rt.binary("-", rt.f(1.0), rt.component_wise("cos", rt.binary("*", clamped, g.PI, 1), width=1), 1), rt.f(0.5), 1)
    ...
    def rgb_to_hsv__vec3(rgb):
        ...
        delta = rt.binary("-", c_max, c_min, 1)
        hue = rt.f(0.0)
        if rt.binary(">", delta, rt.f(0.0)):
            if rt.binary("==", c_max, rt.swizzle(rgb, "x")):
                segment = rt.binary("/", rt.binary("-", rt.swizzle(rgb, "y"), rt.swizzle(rgb, "z"), 1), delta, 1)
                if rt.binary("<", segment, rt.f(0.0)):
                    segment = rt.binary("+", segment, rt.f(6.0), 1)
                hue = segment
            else:
                if rt.binary("==", c_max, rt.swizzle(rgb, "y")):
                    hue = rt.binary("+", rt.binary("/", rt.binary("-", rt.swizzle(rgb, "z"), rt.swizzle(rgb, "x"), 1), delta, 1), rt.f(2.0), 1)
                else:
                    hue = rt.binary("+", rt.binary("/", rt.binary("-", rt.swizzle(rgb, "x"), rt.swizzle(rgb, "y"), 1), delta, 1), rt.f(4.0), 1)
            hue = wrap_unit__float(rt.binary("/", hue, rt.f(6.0), 1))
        saturation = (rt.binary("/", delta, c_max, 1) if rt.binary("!=", c_max, rt.f(0.0)) else rt.f(0.0))
    ...
    def hsv_to_rgb__vec3(hsv):
        ...
        r_comp = clamp01__float(rt.binary("-", rt.component_wise("abs", rt.binary("-", dh, rt.f(3.0), 1), width=1), rt.f(1.0), 1))
        g_comp = clamp01__float(rt.binary("+", rt.unary("-", rt.component_wise("abs", rt.binary("-", dh, rt.f(2.0), 1), width=1)), rt.f(2.0), 1))
        b_comp = clamp01__float(rt.binary("+", rt.unary("-", rt.component_wise("abs", rt.binary("-", dh, rt.f(4.0), 1), width=1)), rt.f(2.0), 1))
        one_minus_s = rt.binary("-", rt.f(1.0), s, 1)
        sr = rt.binary("*", s, r_comp, 1)
        sg = rt.binary("*", s, g_comp, 1)
        sb = rt.binary("*", s, b_comp, 1)
        r = clamp01__float(rt.binary("*", rt.binary("+", one_minus_s, sr, 1), v, 1))
        g = clamp01__float(rt.binary("*", rt.binary("+", one_minus_s, sg, 1), v, 1))
        b = clamp01__float(rt.binary("*", rt.binary("+", one_minus_s, sb, 1), v, 1))
        return rt.construct(3, r, g, b)
    def adjust_hue__vec3_float(color, amount):
        color = rt.copy(color)
        hsv = rgb_to_hsv__vec3(color)
        hsv = rt.assign_swizzle(hsv, "x", wrap_unit__float(rt.binary("+", rt.swizzle(hsv, "x"), amount, 1)))
    ...
    def adjust_saturation__vec3_float(color, amount):
        color = rt.copy(color)
        hsv = rgb_to_hsv__vec3(color)
        hsv = rt.assign_swizzle(hsv, "y", clamp01__float(rt.binary("*", rt.swizzle(hsv, "y"), amount, 1)))
        hsv = rt.assign_swizzle(hsv, "z", clamp01__float(rt.swizzle(hsv, "z")))
        return rt.component_wise("clamp", rt.construct(3, hsv_to_rgb__vec3(hsv)), rt.construct(3, rt.f(0.0)), rt.construct(3, rt.f(1.0)), width=3)
    ...
    def get_scanline_base_values__float_float(time, speed):
        time_scaled = rt.binary("*", rt.binary("*", time, speed, 1), rt.f(0.1), 1)
        noise_seed = rt.binary("+", rt.f(19.37), rt.binary("*", _u_seed, rt.f(31.0), 1), 1)
    ...
    def get_scanline_value_interpolated__float_float_vec2_float(y, height, base_values, pixels_per_bar):
        ...
        y_scaled = rt.binary("/", y, pixels_per_bar, 1)
        scanline_index = rt.binary("%", rt.construct(1, rt.component_wise("floor", y_scaled, width=1)), rt.i(2), 1)
    ...
    def sample_scanline_bilinear__float_float_float_float_vec2_float(sample_x, sample_y, width, height, base_values, pixels_per_bar):
        base_values = rt.copy(base_values)
        wrapped_x = rt.binary("-", sample_x, rt.binary("*", rt.component_wise("floor", rt.binary("/", sample_x, width, 1), width=1), width, 1), 1)
        wrapped_y = rt.binary("-", sample_y, rt.binary("*", rt.component_wise("floor", rt.binary("/", sample_y, height, 1), width=1), height, 1), 1)
        if rt.binary("<", wrapped_x, rt.f(0.0)):
            wrapped_x = rt.binary("+", wrapped_x, width, 1)
        if rt.binary("<", wrapped_y, rt.f(0.0)):
            wrapped_y = rt.binary("+", wrapped_y, height, 1)
        wrapped_x = rt.component_wise("clamp", wrapped_x, rt.f(0.0), rt.binary("-", width, rt.f(1.0), 1), width=1)
        wrapped_y = rt.component_wise("clamp", wrapped_y, rt.f(0.0), rt.binary("-", height, rt.f(1.0), 1), width=1)
        x0 = rt.component_wise("floor", wrapped_x, width=1)
        y0 = rt.component_wise("floor", wrapped_y, width=1)
        x1 = rt.component_wise("min", rt.binary("+", x0, rt.f(1.0), 1), rt.binary("-", width, rt.f(1.0), 1), width=1)
        y1 = rt.component_wise("min", rt.binary("+", y0, rt.f(1.0), 1), rt.binary("-", height, rt.f(1.0), 1), width=1)
        x_fract = rt.component_wise("clamp", rt.binary("-", wrapped_x, x0, 1), rt.f(0.0), rt.f(1.0), width=1)
        y_fract = rt.component_wise("clamp", rt.binary("-", wrapped_y, y0, 1), rt.f(0.0), rt.f(1.0), width=1)
    ...
    def main__void():
        ...
        if rt.binary("==", alphaVal, rt.f(0.0)):
            g.fragColor = rt.texel_fetch(_u_inputTex, cpu_ivec2__float_float(rt.construct(1, rt.swizzle(global_id, "x")), rt.construct(1, rt.swizzle(global_id, "y"))), rt.i(0))
            return
        ...
        width_f = rt.component_wise("max", rt.binary("/", rt.swizzle(fullRes, "x"), rs, 1), rt.f(1.0), width=1)
        height_f = rt.component_wise("max", rt.binary("/", rt.swizzle(fullRes, "y"), rs, 1), rt.f(1.0), width=1)
        _local_time_1 = _u_time
        _local_speed_1 = _u_speed
        x = rt.binary("/", rt.binary("+", rt.swizzle(global_id, "x"), rt.swizzle(_u_tileOffset, "x"), 1), rs, 1)
        y = rt.binary("/", rt.binary("+", rt.swizzle(global_id, "y"), rt.swizzle(_u_tileOffset, "y"), 1), rs, 1)
        displacement = rt.f(0.0625)
        freq = freq_for_shape__float_float_float(rt.f(2.0), width_f, height_f)
        base_offsets = compute_lens_offsets__vec2_float_float_vec2_float_float_float(rt.construct(2, x, y), width_f, height_f, freq, _local_time_1, _local_speed_1, displacement)
        scanline_base = get_scanline_base_values__float_float(_local_time_1, _local_speed_1)
        ppb = rt.f(2.5)
        scan_value = sample_scanline_bilinear__float_float_float_float_vec2_float(rt.binary("+", x, rt.swizzle(base_offsets, "x"), 1), rt.binary("+", y, rt.swizzle(base_offsets, "y"), 1), width_f, height_f, scanline_base, ppb)
        base_sample = rt.texel_fetch(_u_inputTex, cpu_ivec2__vec2(rt.swizzle(global_id, "xy")), rt.i(0))
        base_color = rt.swizzle(base_sample, "xyz")
        _local_alpha_1 = rt.swizzle(base_sample, "w")
        color = rt.component_wise("mix", base_color, rt.binary("*", rt.binary("+", base_color, scan_value, 3), scan_value, 3), rt.f(0.5), width=3)
        ...
        if rt.binary(">=", rt.f(4.0), rt.f(2.5)):
            seed_base = rt.binary("+", rt.f(17.0), rt.binary("*", _u_seed, rt.f(73.0), 1), 1)
            displacement_base = rt.binary("+", rt.f(0.0125), rt.binary("*", random_scalar__float(rt.binary("+", seed_base, rt.f(0.37), 1)), rt.f(0.00625), 1), 1)
            simplex_value = random_scalar__float(rt.binary("+", seed_base, rt.f(0.73), 1))
            displacement_pixels = rt.binary("*", rt.binary("*", displacement_base, width_f, 1), simplex_value, 1)
            ...
            gradient = rt.component_wise("clamp", rt.binary("/", x, rt.binary("-", width_f, rt.f(1.0), 1), 1), rt.f(0.0), rt.f(1.0), width=1)
            hue_shift = rt.binary("-", rt.binary("*", random_scalar__float(rt.binary("+", seed_base, rt.f(1.91), 1)), rt.f(0.25), 1), rt.f(0.125), 1)
            red_x = rt.component_wise("min", rt.binary("+", x, displacement_pixels, 1), rt.binary("-", width_f, rt.f(1.0), 1), width=1)
            red_x = blend_linear__float_float_float(red_x, x, gradient)
            red_sample_x = blend_cosine__float_float_float(x, red_x, aber_mask)
            red_sample_global_x = rt.binary("*", red_sample_x, _u_renderScale, 1)
            red_sample_local_x = rt.binary("-", red_sample_global_x, rt.swizzle(_u_tileOffset, "x"), 1)
            red_base_col = rt.swizzle(rt.texel_fetch(_u_inputTex, cpu_ivec2__float_float(rt.construct(1, red_sample_local_x), rt.construct(1, rt.swizzle(global_id, "y"))), rt.i(0)), "xyz")
            red_offsets = compute_lens_offsets__vec2_float_float_vec2_float_float_float(rt.construct(2, red_sample_x, y), width_f, height_f, freq, _local_time_1, _local_speed_1, displacement)
            red_scan_val = sample_scanline_bilinear__float_float_float_float_vec2_float(rt.binary("+", red_sample_x, rt.swizzle(red_offsets, "x"), 1), rt.binary("+", y, rt.swizzle(red_offsets, "y"), 1), width_f, height_f, scanline_base, ppb)
            red_blended = rt.component_wise("mix", red_base_col, rt.binary("*", rt.binary("+", red_base_col, red_scan_val, 3), red_scan_val, 3), rt.f(0.5), width=3)
            green_blended = color
            blue_x = rt.component_wise("max", rt.binary("-", x, displacement_pixels, 1), rt.f(0.0), width=1)
            blue_x = blend_linear__float_float_float(x, blue_x, gradient)
            blue_sample_x = blend_cosine__float_float_float(x, blue_x, aber_mask)
            blue_sample_global_x = rt.binary("*", blue_sample_x, _u_renderScale, 1)
            blue_sample_local_x = rt.binary("-", blue_sample_global_x, rt.swizzle(_u_tileOffset, "x"), 1)
            blue_base_col = rt.swizzle(rt.texel_fetch(_u_inputTex, cpu_ivec2__float_float(rt.construct(1, blue_sample_local_x), rt.construct(1, rt.swizzle(global_id, "y"))), rt.i(0)), "xyz")
            blue_offsets = compute_lens_offsets__vec2_float_float_vec2_float_float_float(rt.construct(2, blue_sample_x, y), width_f, height_f, freq, _local_time_1, _local_speed_1, displacement)
            blue_scan_val = sample_scanline_bilinear__float_float_float_float_vec2_float(rt.binary("+", blue_sample_x, rt.swizzle(blue_offsets, "x"), 1), rt.binary("+", y, rt.swizzle(blue_offsets, "y"), 1), width_f, height_f, scanline_base, ppb)
            blue_blended = rt.component_wise("mix", blue_base_col, rt.binary("*", rt.binary("+", blue_base_col, blue_scan_val, 3), blue_scan_val, 3), rt.f(0.5), width=3)
            ...
            vignette_alpha = rt.binary("*", random_scalar__float(rt.binary("+", seed_base, rt.f(3.17), 1)), rt.f(0.175), 1)
        ...
        local_mean = rt.binary("*", rt.binary("+", rt.binary("+", rt.swizzle(color, "x"), rt.swizzle(color, "y"), 1), rt.swizzle(color, "z"), 1), g.INV_THREE, 1)
        color = rt.component_wise("clamp", rt.binary("+", rt.binary("*", rt.binary("-", color, local_mean, 3), rt.f(1.25), 3), local_mean, 3), rt.construct(3, rt.f(0.0)), rt.construct(3, rt.f(1.0)), width=3)
