def run_pixel(ctx, out):
    ...
    def getZoneMeta__int(z):
        return _u_data[int(rt.binary("+", rt.i(2), z, 1))]
    def getZonePack__int_int(zoneIdx, pairIdx):
        return _u_data[int(rt.binary("+", rt.binary("+", rt.i(10), rt.binary("*", zoneIdx, rt.i(32), 1), 1), pairIdx, 1))]
    def getVert__int_int(zoneIdx, vertIdx):
        _packed = getZonePack__int_int(zoneIdx, rt.binary("/", vertIdx, rt.i(2), 1))
        return (rt.swizzle(_packed, "xy") if rt.binary("==", rt.binary("%", vertIdx, rt.i(2), 1), rt.i(0)) else rt.swizzle(_packed, "zw"))
    def getZoneCount__int(z):
        return rt.construct(1, rt.swizzle(getZoneMeta__int(z), "x"))
    def getZoneActive__int(z):
        return rt.construct(1, rt.binary("+", rt.swizzle(getZoneMeta__int(z), "y"), rt.f(0.5), 1))
    ...
    def pointInZone__vec2_int(p, zoneIdx):
        ...
        if rt.binary("<", n, rt.i(3)):
            return False
        inside = False
        prev = getVert__int_int(zoneIdx, rt.binary("-", n, rt.i(1), 1))
        i = rt.i(0)
        _for0_first = True
        for _for0 in range(1048576):
            ...
            if crosses:
                xCross = rt.binary("+", rt.binary("/", rt.binary("*", rt.binary("-", rt.swizzle(prev, "x"), rt.swizzle(cur, "x"), 1), rt.binary("-", rt.swizzle(p, "y"), rt.swizzle(cur, "y"), 1), 1), rt.binary("+", rt.binary("-", rt.swizzle(prev, "y"), rt.swizzle(cur, "y"), 1), rt.f(1e-9), 1), 1), rt.swizzle(cur, "x"), 1)
    ...
    def distToZoneEdge__vec2_int(p, zoneIdx):
        ...
        for _for1 in range(1048576):
            ...
            if rt.binary(">=", i, n):
                break
            cur = getVert__int_int(zoneIdx, i)
            ab = rt.binary("-", cur, prev, 2)
            len2 = rt.component_wise("max", rt.dot(ab, ab), rt.f(1e-9), width=1)
            t = rt.component_wise("clamp", rt.binary("/", rt.dot(rt.binary("-", p, prev, 2), ab), len2, 1), rt.f(0.0), rt.f(1.0), width=1)
            closest = rt.binary("+", prev, rt.binary("*", t, ab, 2), 2)
            d = rt.component_wise("min", d, rt.length(rt.binary("-", p, closest, 2)), width=1)
            prev = cur
        return d
    def main__void():
        globalCoord = rt.binary("+", rt.swizzle(ctx.frag_coord, "xy"), _u_tileOffset, 2)
        globalScreen = rt.binary("/", rt.binary("+", rt.swizzle(ctx.frag_coord, "xy"), _u_tileOffset, 2), _u_fullResolution, 2)
        p = rt.construct(2, rt.swizzle(globalScreen, "x"), rt.binary("-", rt.f(1.0), rt.swizzle(globalScreen, "y"), 1))
        sampleUv = rt.binary("/", globalCoord, _u_fullResolution, 2)
        header = _u_data[int(rt.i(0))]
        controls = _u_data[int(rt.i(1))]
        bgColor = rt.swizzle(header, "xyz")
        bgAlpha = rt.swizzle(header, "w")
        activeCount = rt.component_wise("min", rt.construct(1, rt.swizzle(controls, "x")), rt.i(8), width=1)
        ...
        for _for2 in range(1048576):
            ...
            edgeWidth = rt.binary("*", smoothEdge, rt.f(0.05), 1)
            ...
            a = rt.binary("*", zAlpha, edge, 1)
            result = rt.construct(4, rt.component_wise("mix", rt.swizzle(result, "rgb"), rt.swizzle(src, "rgb"), a, width=3), rt.component_wise("max", rt.swizzle(result, "a"), rt.binary("*", rt.swizzle(src, "a"), a, 1), width=1))
        g.fragColor = result
    main__void()
    _c = g.fragColor
