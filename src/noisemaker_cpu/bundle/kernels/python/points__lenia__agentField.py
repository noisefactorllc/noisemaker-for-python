def run_pixel(ctx, out):
    ...
    def main__void():
        ...
        Ux_plus = rt.swizzle(rt.texture(_u_fieldTex, rt.component_wise("fract", rt.binary("+", uv, rt.array([rt.swizzle(texelSize, "x"), rt.f(0.0)]), 2, "float"), width=2)), "r")
        Ux_minus = rt.swizzle(rt.texture(_u_fieldTex, rt.component_wise("fract", rt.binary("-", uv, rt.array([rt.swizzle(texelSize, "x"), rt.f(0.0)]), 2, "float"), width=2)), "r")
        Uy_plus = rt.swizzle(rt.texture(_u_fieldTex, rt.component_wise("fract", rt.binary("+", uv, rt.array([rt.f(0.0), rt.swizzle(texelSize, "y")]), 2, "float"), width=2)), "r")
        Uy_minus = rt.swizzle(rt.texture(_u_fieldTex, rt.component_wise("fract", rt.binary("-", uv, rt.array([rt.f(0.0), rt.swizzle(texelSize, "y")]), 2, "float"), width=2)), "r")
