def run_pixel(ctx, out):
    ...
    def main__void():
        ...
        strength = rt.binary("*", _u_detail, rt.f(0.2), 1)
        ...
        if rt.binary("==", _u_MODE, rt.i(0)):
            ...
            if rt.binary("==", _u_MODE, rt.i(1)):
                hhC = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.35), rt.f(0.65), hC, width=1), 1)
                hhR = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.35), rt.f(0.65), hR, width=1), 1)
                hhT = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.35), rt.f(0.65), hT, width=1), 1)
                ...
                if rt.binary("==", _u_MODE, rt.i(2)):
                    threshold = rt.binary("/", _u_balance, rt.f(100.0), 1)
                    m = rt.component_wise("step", threshold, hC, width=1)
                    sheet = rt.component_wise("mix", rt.binary("+", rt.binary("*", _u_inkColor, rt.f(0.9), 3), rt.f(0.1), 3), _u_paperColor, m, width=3)
                    shade = reliefShade__float_float_float_float_float(hC, hR, hT, strength, _u_lightAngle)
                    gradMag = rt.length(rt.construct(2, rt.binary("-", hR, hC, 1), rt.binary("-", hT, hC, 1)))
                    bandHeight = rt.component_wise("max", rt.binary("*", gradMag, rt.f(2.0), 1), rt.f(1e-5), width=1)
                    edge = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.0), bandHeight, rt.component_wise("abs", rt.binary("-", hC, threshold, 1), width=1), width=1), 1)
                    beveled = rt.component_wise("clamp", rt.binary("*", sheet, rt.component_wise("mix", rt.f(0.6), rt.f(1.4), shade, width=1), 3), rt.f(0.0), rt.f(1.0), width=3)
                    sheetOut = rt.component_wise("mix", sheet, beveled, edge, width=3)
                    globalCoord = rt.binary("+", rt.swizzle(ctx.frag_coord, "xy"), _u_tileOffset, 2)
                    grain = rt.binary("*", rt.binary("*", rt.binary("-", hash12__vec2(rt.component_wise("floor", globalCoord, width=2)), rt.f(0.5), 1), rt.binary("/", _u_graininess, rt.f(100.0), 1), 1), rt.f(0.15), 1)
                    outColor = rt.component_wise("clamp", rt.binary("+", sheetOut, rt.construct(3, grain), 3), rt.f(0.0), rt.f(1.0), width=3)
