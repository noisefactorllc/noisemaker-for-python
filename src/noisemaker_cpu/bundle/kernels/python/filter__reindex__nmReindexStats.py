def run_pixel(ctx, out):
    ...
    def main__void():
        fragCoord = cpu_ivec2__vec2(rt.swizzle(ctx.frag_coord, "xy"))
        localX = rt.binary("%", rt.swizzle(fragCoord, "x"), g.TILE_SIZE, 1)
        localY = rt.binary("%", rt.swizzle(fragCoord, "y"), g.TILE_SIZE, 1)
        if rt.binary("||", rt.binary("!=", localX, rt.i(0)), rt.binary("!=", localY, rt.i(0))):
        ...
        for _for0 in range(1048576):
            ...
            if not (rt.binary("<", oy, g.TILE_SIZE)):
                break
            py = rt.binary("+", rt.swizzle(tileOrigin, "y"), oy, 1)
            ...
            for _for1 in range(1048576):
                ...
                if not (rt.binary("<", ox, g.TILE_SIZE)):
                    break
                px = rt.binary("+", rt.swizzle(tileOrigin, "x"), ox, 1)
                if rt.binary(">=", px, rt.swizzle(texSize, "x")):
                    break
                texel = rt.texel_fetch(_u_inputTex, cpu_ivec2__float_float(px, py), rt.i(0))
