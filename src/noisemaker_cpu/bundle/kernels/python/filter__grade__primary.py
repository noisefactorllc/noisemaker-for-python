def run_pixel(ctx, out):
    ...
    def shadowWeight__float(luma):
        return rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.0), rt.f(0.5), luma, width=1), 1, "float")
    ...
    def midtoneWeight__float(luma):
        return rt.binary("-", rt.f(1.0), rt.binary("*", rt.component_wise("abs", rt.binary("-", luma, rt.f(0.5), 1, "float"), width=1), rt.f(2.0), 1, "float"), 1, "float")
    ...
    def applyTonalRanges__vec3_float_float_float_float(rgb, highlights, shadows, whites, blacks):
        ...
        lumaAdjust = rt.binary("+", lumaAdjust, rt.binary("*", rt.binary("*", whites, wWeight, 1, "float"), rt.f(0.3), 1, "float"), 1, "float")
        lumaAdjust = rt.binary("+", lumaAdjust, rt.binary("*", rt.binary("*", blacks, bWeight, 1, "float"), rt.f(0.3), 1, "float"), 1, "float")
    ...
    def applyContrast__vec3_float(rgb, contrast):
        ...
        chroma = rt.binary("-", rgb, luma, 3, "float")
        pivot = rt.f(0.5)
        factor = rt.binary("+", rt.f(1.0), contrast, 1, "float")
    ...
    def applyCurve__vec3_float_float_float(rgb, shadowLift, midGamma, highGain):
        ...
        lift = rt.binary("*", rt.binary("*", shadowLift, sW, 1, "float"), rt.f(0.2), 1, "float")
        gamma = rt.binary("-", rt.f(1.0), rt.binary("*", rt.binary("*", midGamma, mW, 1, "float"), rt.f(0.3), 1, "float"), 1, "float")
        gain = rt.binary("+", rt.f(1.0), rt.binary("*", rt.binary("*", highGain, hW, 1, "float"), rt.f(0.5), 1, "float"), 1, "float")
    ...
    def main__void():
        ...
        straight = (rt.binary("/", rt.swizzle(color, "rgb"), rt.swizzle(color, "a"), 3, "float") if rt.binary(">", rt.swizzle(color, "a"), rt.f(0.0)) else rt.construct(3, rt.f(0.0)))
        rgb = srgbToLinear__vec3(straight)
        rgb[:] = applyWhiteBalance__vec3_float_float(rgb, _u_temperature, _u_tint)
        rgb[:] = rt.binary("*", rgb, rt.component_wise("pow", rt.f(2.0), _u_exposure, width=1), 3, "float")
