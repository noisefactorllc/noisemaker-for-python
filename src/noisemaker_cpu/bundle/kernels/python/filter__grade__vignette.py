def run_pixel(ctx, out):
    ...
    def computeVignette__vec2_vec2_float_float_float(uv, aspectRatio, midpoint, roundness, feather):
        ...
        if rt.binary(">", roundness, rt.f(0.0)):
            ...
            scale = rt.component_wise("mix", aspectRatio, rt.binary("*", aspectRatio, rt.construct(2, rt.binary("+", rt.f(1.0), rt.component_wise("abs", roundness, width=1), 1), rt.binary("-", rt.f(1.0), rt.binary("*", rt.component_wise("abs", roundness, width=1), rt.f(0.5), 1), 1)), 2), rt.unary("-", roundness), width=2)
        centered = rt.binary("*", centered, scale, 2)
        dist = rt.binary("*", rt.length(centered), rt.f(2.0), 1)
        inner = rt.binary("-", midpoint, rt.binary("*", feather, rt.f(0.5), 1), 1)
        outer = rt.binary("+", midpoint, rt.binary("*", feather, rt.f(0.5), 1), 1)
        return rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", inner, outer, dist, width=1), 1)
    def applyVignette__vec3_float_float_float(rgb, vignetteMask, amount, highlightProtect):
        ...
        if rt.binary("<", rt.component_wise("abs", amount, width=1), rt.f(0.001)):
            return rgb
        darken = rt.binary("-", rt.f(1.0), rt.binary("*", rt.binary("-", rt.f(1.0), vignetteMask, 1), rt.component_wise("abs", amount, width=1), 1), 1)
        if rt.binary(">", highlightProtect, rt.f(0.0)):
            luma = rt.dot(rgb, g.LUMA_WEIGHTS)
            protection = rt.binary("*", rt.component_wise("smoothstep", rt.f(0.5), rt.f(1.0), luma, width=1), highlightProtect, 1)
            darken = rt.component_wise("mix", darken, rt.f(1.0), protection, width=1)
        if rt.binary(">", amount, rt.f(0.0)):
            return rt.binary("*", rgb, darken, 3)
        else:
            return rt.binary("-", rt.f(1.0), rt.binary("*", rt.binary("-", rt.f(1.0), rgb, 3), darken, 3), 3)
    def main__void():
        ...
        if rt.binary(">", rt.swizzle(fullRes, "x"), rt.swizzle(fullRes, "y")):
            aspectRatio = rt.construct(2, rt.binary("/", rt.swizzle(fullRes, "x"), rt.swizzle(fullRes, "y"), 1), rt.f(1.0))
        else:
            aspectRatio = rt.construct(2, rt.f(1.0), rt.binary("/", rt.swizzle(fullRes, "y"), rt.swizzle(fullRes, "x"), 1))
