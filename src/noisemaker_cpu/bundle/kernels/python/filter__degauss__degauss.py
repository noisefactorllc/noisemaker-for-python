def run_pixel(ctx, out):
    ...
    def simplex_noise__vec3(v):
        ...
        ns = rt.binary("-", rt.binary("*", n_, rt.construct(3, rt.swizzle(D, "w"), rt.swizzle(D, "y"), rt.swizzle(D, "z")), 3, "float"), rt.array([rt.swizzle(D, "x"), rt.swizzle(D, "z"), rt.swizzle(D, "x")]), 3, "float")
        ...
        a0 = rt.binary("+", rt.construct(4, rt.swizzle(b0, "x"), rt.swizzle(b0, "z"), rt.swizzle(b0, "y"), rt.swizzle(b0, "w")), rt.binary("*", rt.construct(4, rt.swizzle(s0, "x"), rt.swizzle(s0, "z"), rt.swizzle(s0, "y"), rt.swizzle(s0, "w")), rt.array([rt.swizzle(sh, "x"), rt.swizzle(sh, "x"), rt.swizzle(sh, "y"), rt.swizzle(sh, "y")]), 4, "float"), 4, "float")
        a1 = rt.binary("+", rt.construct(4, rt.swizzle(b1, "x"), rt.swizzle(b1, "z"), rt.swizzle(b1, "y"), rt.swizzle(b1, "w")), rt.binary("*", rt.construct(4, rt.swizzle(s1, "x"), rt.swizzle(s1, "z"), rt.swizzle(s1, "y"), rt.swizzle(s1, "w")), rt.array([rt.swizzle(sh, "z"), rt.swizzle(sh, "z"), rt.swizzle(sh, "w"), rt.swizzle(sh, "w")]), 4, "float"), 4, "float")
    ...
    def singularity_mask__vec2_float_float(uv, width, height):
        ...
        if (bool(rt.binary("<=", width, rt.f(0.0))) or bool(rt.binary("<=", height, rt.f(0.0)))):
            return rt.f(0.0)
        delta = rt.component_wise("abs", rt.binary("-", uv, rt.f(0.5), 2, "float"), width=2)
    ...
    def warped_channel_value__uint_uvec2_vec2_float_float_vec2_float_float_float_float(channel, coord, base_pos, width, height, freq, displacement, mask, time, speed):
        ...
        offset = rt.binary("*", rt.binary("*", rt.construct(2, rt.component_wise("cos", angle, width=1), rt.component_wise("sin", angle, width=1)), displacement, 2, "float"), rt.array([rt.swizzle(_u_resolution, "x"), rt.swizzle(_u_resolution, "y")]), 2, "float")
    ...
    def main__void():
        ...
        uv = rt.binary("/", rt.binary("+", rt.construct(2, rt.binary("+", rt.construct(1, rt.swizzle(global_id, "x")), rt.swizzle(_u_tileOffset, "x"), 1, "float"), rt.binary("+", rt.construct(1, rt.swizzle(global_id, "y")), rt.swizzle(_u_tileOffset, "y"), 1, "float")), rt.f(0.5), 2, "float"), rt.array([rt.component_wise("max", width_f, rt.f(1.0), width=1), rt.component_wise("max", height_f, rt.f(1.0), width=1)]), 2, "float")
