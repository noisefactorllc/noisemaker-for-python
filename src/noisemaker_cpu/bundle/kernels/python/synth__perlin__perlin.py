def run_pixel(ctx, out):
    ...
    def hash3__vec3(p):
        p = rt.copy(p)
        p = rt.binary("+", p, rt.binary("*", _u_seed, rt.f(0.1), 1, "float"), 3, "float")
        q = cpu_uvec3__vec3(rt.binary("+", cpu_ivec3__vec3(rt.binary("*", p, rt.f(1000.0), 3, "float")), rt.i(65536), 3, "float"))
        q = rt.binary("+", rt.binary("*", q, rt.i(1664525), 3, "float"), rt.i(1013904223), 3, "float")
        q = rt.assign_swizzle(q, "x", rt.binary("+", rt.swizzle(q, "x"), rt.binary("*", rt.swizzle(q, "y"), rt.swizzle(q, "z"), 1, "float"), 1))
        q = rt.assign_swizzle(q, "y", rt.binary("+", rt.swizzle(q, "y"), rt.binary("*", rt.swizzle(q, "z"), rt.swizzle(q, "x"), 1, "float"), 1))
        q = rt.assign_swizzle(q, "z", rt.binary("+", rt.swizzle(q, "z"), rt.binary("*", rt.swizzle(q, "x"), rt.swizzle(q, "y"), 1, "float"), 1))
        q = rt.binary("^", q, rt.binary(">>", q, rt.i(16), 3, "int"), 3)
        q = rt.assign_swizzle(q, "x", rt.binary("+", rt.swizzle(q, "x"), rt.binary("*", rt.swizzle(q, "y"), rt.swizzle(q, "z"), 1, "float"), 1))
        q = rt.assign_swizzle(q, "y", rt.binary("+", rt.swizzle(q, "y"), rt.binary("*", rt.swizzle(q, "z"), rt.swizzle(q, "x"), 1, "float"), 1))
        q = rt.assign_swizzle(q, "z", rt.binary("+", rt.swizzle(q, "z"), rt.binary("*", rt.swizzle(q, "x"), rt.swizzle(q, "y"), 1, "float"), 1))
        return rt.binary("/", rt.construct(1, rt.binary("^", rt.binary("^", rt.swizzle(q, "x"), rt.swizzle(q, "y"), 1, "int"), rt.swizzle(q, "z"), 1, "int")), rt.f(4294967295.0), 1, "float")
    def grad3__vec3(p):
        p = rt.copy(p)
        h1 = hash3__vec3(p)
        h2 = hash3__vec3(rt.binary("+", p, rt.f(127.1), 3, "float"))
        h3 = hash3__vec3(rt.binary("+", p, rt.f(269.5), 3, "float"))
        g = rt.construct(3, rt.binary("-", rt.binary("*", h1, rt.f(2.0), 1, "float"), rt.f(1.0), 1, "float"), rt.binary("-", rt.binary("*", h2, rt.f(2.0), 1, "float"), rt.f(1.0), 1, "float"), rt.binary("-", rt.binary("*", h3, rt.f(2.0), 1, "float"), rt.f(1.0), 1, "float"))
    ...
    def grid2D__vec2_vec2_float_float(st, cell, timeAngle, channelOffset):
        st = rt.copy(st)
        cell = rt.copy(cell)
        angle = rt.binary("*", rt.swizzle(prng__vec3(rt.construct(3, rt.binary("+", cell, _u_seed, 2, "float"), rt.f(1.0))), "r"), g.TAU, 1, "float")
        angle = rt.binary("+", angle, rt.binary("+", timeAngle, rt.binary("*", channelOffset, g.TAU, 1, "float"), 1, "float"), 1)
    ...
    def fbm2D__vec2_float_float_int(st, timeAngle, channelOffset, ridgedMode):
        ...
        for _for0 in range(1048576):
            ...
            sum = rt.binary("+", sum, rt.binary("*", n, amplitude, 1, "float"), 1)
    ...
    def domainWarp2D__vec2_float_int_float_float(st, timeAngle, iterations, wScale, wIntensity):
        ...
        for _for2 in range(1048576):
            ...
            p = rt.binary("+", p, rt.binary("*", rt.construct(2, nx, ny), disp, 2, "float"), 2)
        return p
