def run_pixel(ctx, out):
    ...
    def snow_fract_vec3__vec3(value):
        value = rt.copy(value)
        return rt.binary("-", value, rt.component_wise("floor", value, width=3), 3)
    def snow_hash__vec3(input_sample):
        input_sample = rt.copy(input_sample)
        scaled = snow_fract_vec3__vec3(rt.binary("*", input_sample, rt.f(0.1031), 3))
        dot_val = rt.dot(scaled, rt.binary("+", rt.swizzle(scaled, "yzx"), rt.construct(3, rt.f(33.33)), 3))
        shifted = rt.binary("+", scaled, dot_val, 3)
        combined = rt.binary("*", rt.binary("+", rt.swizzle(shifted, "x"), rt.swizzle(shifted, "y"), 1), rt.swizzle(shifted, "z"), 1)
        fractional = rt.binary("-", combined, rt.component_wise("floor", combined, width=1), 1)
    ...
    def snow_noise__vec2_float_float_vec3(coord, time, speed, seed):
        ...
        base_sample = rt.construct(3, rt.binary("+", rt.swizzle(coord, "x"), rt.swizzle(seed, "x"), 1), rt.binary("+", rt.swizzle(coord, "y"), rt.swizzle(seed, "y"), 1), rt.binary("+", z_base, rt.swizzle(seed, "z"), 1))
        base_value = snow_hash__vec3(base_sample)
        if rt.binary("||", rt.binary("==", speed, rt.f(0.0)), rt.binary("==", time, rt.f(0.0))):
            return base_value
        time_seed = rt.binary("+", seed, g.TIME_SEED_OFFSETS, 3)
        time_sample = rt.construct(3, rt.binary("+", rt.swizzle(coord, "x"), rt.swizzle(time_seed, "x"), 1), rt.binary("+", rt.swizzle(coord, "y"), rt.swizzle(time_seed, "y"), 1), rt.binary("+", rt.f(1.0), rt.swizzle(time_seed, "z"), 1))
    ...
    def main__void():
        ...
        coords = cpu_ivec2__float_float(rt.construct(1, rt.swizzle(ctx.frag_coord, "x")), rt.construct(1, rt.swizzle(ctx.frag_coord, "y")))
        ...
        if rt.binary("==", alphaVal, rt.f(0.0)):
            g.fragColor = texel
            return
        pixelCoord = rt.construct(2, rt.binary("+", rt.swizzle(ctx.frag_coord, "x"), rt.swizzle(_u_tileOffset, "x"), 1), rt.binary("+", rt.swizzle(ctx.frag_coord, "y"), rt.swizzle(_u_tileOffset, "y"), 1))
        ...
        d = rt.component_wise("max", rt.binary("*", _u_density, rt.f(0.01), 1), rt.f(0.0001), width=1)
        exponent = rt.binary("/", rt.binary("-", rt.f(1.0), d, 1), d, 1)
        limiter_mask = rt.binary("*", rt.component_wise("pow", rt.component_wise("min", limiter_value, rt.f(0.99), width=1), exponent, width=1), alphaVal, 1)
