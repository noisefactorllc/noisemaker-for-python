def run_pixel(ctx, out):
    ...
    def main__void():
        ...
        band = rt.binary("-", lumSrc, lumBlur, 1)
        edgeGain = rt.component_wise("mix", rt.f(4.0), rt.f(18.0), rt.binary("/", _u_darkness, rt.f(100.0), 1), width=1)
        edgeInk = rt.component_wise("clamp", rt.binary("*", rt.component_wise("abs", band, width=1), edgeGain, 1), rt.f(0.0), rt.f(1.0), width=1)
        toneHi = rt.component_wise("mix", rt.f(0.35), rt.f(0.68), rt.binary("/", _u_darkness, rt.f(100.0), 1), width=1)
        toneLo = rt.binary("-", toneHi, rt.f(0.26), 1)
        toneInk = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", toneLo, toneHi, lumSrc, width=1), 1)
        ink = rt.component_wise("clamp", rt.component_wise("max", edgeInk, toneInk, width=1), rt.f(0.0), rt.f(1.0), width=1)
        outColor = tonemap2__float_vec3_vec3(rt.binary("-", rt.f(1.0), ink, 1), _u_inkColor, _u_paperColor)
