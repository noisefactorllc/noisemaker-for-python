def run_pixel(ctx, out):
    ...
    def main__void():
        ...
        centered = rt.binary("*", rt.binary("-", globalUV, rt.f(0.5), 2), rt.f(2.0), 2)
        centered = rt.assign_swizzle(centered, "x", rt.binary("*", rt.swizzle(centered, "x"), aspect, 1))
        ...
        extent = rt.binary("+", rt.binary("+", rt.binary("*", aspect, rt.component_wise("abs", s, width=1), 1), rt.component_wise("abs", c, width=1), 1), _u_softness, 1)
        animPos = _u_position
        flipCycle = False
        if rt.binary(">", _u_speed, rt.f(0.0)):
            cycle = rt.binary("*", rt.binary("*", _u_time, _u_speed, 1), rt.f(2.0), 1)
            ...
            animPos = rt.binary("-", rt.binary("*", rt.binary("*", t, extent, 1), rt.f(2.0), 1), extent, 1)
        d = rt.binary("-", rt.swizzle(rotated, "y"), animPos, 1)
        halfSoft = rt.component_wise("max", rt.binary("*", _u_softness, rt.f(0.5), 1), rt.f(0.001), width=1)
