def run_pixel(ctx, out):
    ...
    def value_noise__vec2_float(coord, s):
        ...
        tr = hash31__vec3(rt.construct(3, rt.binary("+", cell, rt.array([rt.f(1.0), rt.f(0.0)]), 2, "float"), s))
        bl = hash31__vec3(rt.construct(3, rt.binary("+", cell, rt.array([rt.f(0.0), rt.f(1.0)]), 2, "float"), s))
        br = hash31__vec3(rt.construct(3, rt.binary("+", cell, rt.f(1.0), 2, "float"), s))
    ...
    def main__void():
        ...
        dropout = (rt.f(1.0) if rt.binary("<", hash21__vec2(rt.binary("+", rt.binary("*", globalUV, _u_fullResolution, 2, "float"), rt.array([rt.binary("+", s, rt.f(37.0), 1, "float"), rt.binary("*", s, rt.f(1.37), 1, "float")]), 2, "float")), rt.f(0.4)) else rt.f(0.0))
        ...
        sparse_mask = (rt.f(1.0) if rt.binary("<", hash21__vec2(rt.binary("+", rt.binary("*", globalUV, _u_fullResolution, 2, "float"), rt.array([rt.binary("+", s, rt.f(113.0), 1, "float"), rt.binary("+", s, rt.f(171.0), 1, "float")]), 2, "float")), rt.f(0.25)) else rt.f(0.0))
