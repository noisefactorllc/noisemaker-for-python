def run_pixel(ctx, out):
    ...
    def simplex3D__vec3(v):
        ...
        v = rt.binary("+", v, rt.binary("*", _u_seed, rt.f(0.1271), 1, "float"), 3)
        ...
        l = rt.binary("-", rt.f(1.0), g, 3, "float")
    ...
    def fbmSimplex3D__vec3(p):
        ...
        for _for0 in range(1048576):
            ...
            sum = rt.binary("+", sum, rt.binary("*", n, amp, 1, "float"), 1)
    ...
    def curlNoise3D__vec3(p):
        p = rt.copy(p)
        eps = rt.f(1.0)
        a = rt.binary("*", rt.binary("/", rt.binary("+", rt.binary("*", rt.component_wise("sin", rt.binary("*", _u_time, rt.f(6.28318), 1, "float"), width=1), _u_speed, 1, "float"), rt.f(1.0), 1, "float"), _u_OCTAVES, 1, "float"), rt.f(0.2), 1, "float")
        b = rt.binary("*", rt.binary("/", rt.binary("+", rt.binary("*", rt.component_wise("cos", rt.binary("*", _u_time, rt.f(6.28318), 1, "float"), width=1), _u_speed, 1, "float"), rt.f(1.0), 1, "float"), _u_OCTAVES, 1, "float"), rt.f(0.2), 1, "float")
