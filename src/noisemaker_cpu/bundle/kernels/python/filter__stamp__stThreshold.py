def run_pixel(ctx, out):
    ...
    def main__void():
        ...
        grain = rt.binary("*", rt.binary("*", rt.binary("-", fbm__vec2(rt.binary("/", globalCoord, rt.f(3.0), 2)), rt.f(0.5), 1), rt.binary("/", _u_roughness, rt.f(100.0), 1), 1), rt.f(0.35), 1)
        t = rt.binary("+", lumBlur, grain, 1)
        b = rt.binary("/", _u_balance, rt.f(100.0), 1)
        aa = rt.binary("+", rt.component_wise("max", rt.component_wise("fwidth", t, width=1), rt.f(0.01), width=1), rt.binary("*", rt.binary("/", _u_roughness, rt.f(100.0), 1), rt.f(0.05), 1), 1)
        m = rt.component_wise("smoothstep", rt.binary("-", b, aa, 1), rt.binary("+", b, aa, 1), t, width=1)
