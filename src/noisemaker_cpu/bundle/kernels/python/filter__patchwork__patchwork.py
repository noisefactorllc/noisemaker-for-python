def run_pixel(ctx, out):
    ...
    def cellAvgColor3x3__vec2(centerPx):
        ...
        sp = rt.binary("*", _u_squareSize, rt.f(0.25), 1)
    ...
    def main__void():
        ...
        relPx = rt.binary("-", globalCoord, imgCenter, 2)
        cellIdxF = rt.component_wise("floor", rt.binary("/", relPx, _u_squareSize, 2), width=2)
        localPx = rt.binary("-", relPx, rt.binary("*", cellIdxF, _u_squareSize, 2), 2)
        cellCenter = rt.binary("+", imgCenter, rt.binary("*", rt.binary("+", cellIdxF, rt.f(0.5), 2), _u_squareSize, 2), 2)
        cellColor = rt.swizzle(cellAvgColor3x3__vec2(cellCenter), "rgb")
        h = lum__vec3(cellColor)
        topFaceShade = rt.binary("+", rt.f(0.9), rt.binary("*", rt.f(0.2), rt.binary("-", h, rt.f(0.5), 1), 1), 1)
        rimPx = rt.binary("*", rt.f(0.15), _u_squareSize, 1)
        dLeft = rt.swizzle(localPx, "x")
        dRight = rt.binary("-", _u_squareSize, rt.swizzle(localPx, "x"), 1)
        dBottom = rt.swizzle(localPx, "y")
        dTop = rt.binary("-", _u_squareSize, rt.swizzle(localPx, "y"), 1)
        ...
        if rt.binary("<", dMin, rimPx):
            ...
            if rt.binary("==", dMin, dLeft):
                neighborIdx = rt.assign_swizzle(neighborIdx, "x", rt.binary("-", rt.swizzle(neighborIdx, "x"), rt.f(1.0), 1))
                ...
                if rt.binary("==", dMin, dRight):
                    ...
                    if rt.binary("==", dMin, dBottom):
                        neighborIdx = rt.assign_swizzle(neighborIdx, "y", rt.binary("-", rt.swizzle(neighborIdx, "y"), rt.f(1.0), 1))
                        edgeNormal = rt.construct(2, rt.f(0.0), rt.unary("-", rt.f(1.0)))
                    else:
                        neighborIdx = rt.assign_swizzle(neighborIdx, "y", rt.binary("+", rt.swizzle(neighborIdx, "y"), rt.f(1.0), 1))
                        edgeNormal = rt.construct(2, rt.f(0.0), rt.f(1.0))
            neighborCenter = rt.binary("+", imgCenter, rt.binary("*", rt.binary("+", neighborIdx, rt.f(0.5), 2), _u_squareSize, 2), 2)
            hNeighbor = lum__vec3(rt.swizzle(cellAvgColor3x3__vec2(neighborCenter), "rgb"))
            dh = rt.binary("-", h, hNeighbor, 1)
            ...
            bevelMul = rt.binary("+", rt.f(1.0), rt.binary("*", rt.binary("*", rt.binary("*", rt.f(0.35), rt.binary("/", _u_relief, rt.f(100.0), 1), 1), rt.component_wise("sign", dh, width=1), 1), signTerm, 1), 1)
        result = rt.component_wise("clamp", rt.binary("*", rt.binary("*", cellColor, topFaceShade, 3), bevelMul, 3), rt.f(0.0), rt.f(1.0), width=3)
