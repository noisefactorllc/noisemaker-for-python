def run_pixel(ctx, out):
    ...
    def rgb2hsv__vec3(c):
        ...
        delta = rt.binary("-", cmax, cmin, 1)
        h = rt.f(0.0)
        if rt.binary(">", delta, rt.f(0.0)):
            if rt.binary("==", cmax, rt.swizzle(c, "r")):
                h = rt.binary("/", rt.component_wise("mod", rt.binary("/", rt.binary("-", rt.swizzle(c, "g"), rt.swizzle(c, "b"), 1), delta, 1), rt.f(6.0), width=1), rt.f(6.0), 1)
            else:
                if rt.binary("==", cmax, rt.swizzle(c, "g")):
                    h = rt.binary("/", rt.binary("+", rt.binary("/", rt.binary("-", rt.swizzle(c, "b"), rt.swizzle(c, "r"), 1), delta, 1), rt.f(2.0), 1), rt.f(6.0), 1)
                else:
                    h = rt.binary("/", rt.binary("+", rt.binary("/", rt.binary("-", rt.swizzle(c, "r"), rt.swizzle(c, "g"), 1), delta, 1), rt.f(4.0), 1), rt.f(6.0), 1)
        s = (rt.binary("/", delta, cmax, 1) if rt.binary(">", cmax, rt.f(0.0)) else rt.f(0.0))
        return rt.construct(3, h, s, cmax)
    def linear2srgb__vec3(lin):
        lin = rt.copy(lin)
        low = rt.binary("*", lin, rt.f(12.92), 3)
        high = rt.binary("-", rt.binary("*", rt.f(1.055), rt.component_wise("pow", rt.component_wise("max", lin, rt.construct(3, rt.f(0.0)), width=3), rt.construct(3, rt.binary("/", rt.f(1.0), rt.f(2.4), 1)), width=3), 3), rt.f(0.055), 3)
    ...
    def srgb2linear__vec3(c):
        c = rt.copy(c)
        low = rt.binary("/", c, rt.f(12.92), 3)
        high = rt.component_wise("pow", rt.binary("/", rt.binary("+", c, rt.f(0.055), 3), rt.f(1.055), 3), rt.construct(3, rt.f(2.4)), width=3)
    ...
    def oklab2linear__vec3(lab):
        lab = rt.copy(lab)
        l_ = rt.binary("+", rt.binary("+", rt.swizzle(lab, "x"), rt.binary("*", rt.f(0.3963377774), rt.swizzle(lab, "y"), 1), 1), rt.binary("*", rt.f(0.2158037573), rt.swizzle(lab, "z"), 1), 1)
        m_ = rt.binary("-", rt.binary("-", rt.swizzle(lab, "x"), rt.binary("*", rt.f(0.1055613458), rt.swizzle(lab, "y"), 1), 1), rt.binary("*", rt.f(0.0638541728), rt.swizzle(lab, "z"), 1), 1)
        s_ = rt.binary("-", rt.binary("-", rt.swizzle(lab, "x"), rt.binary("*", rt.f(0.0894841775), rt.swizzle(lab, "y"), 1), 1), rt.binary("*", rt.f(1.2914855480), rt.swizzle(lab, "z"), 1), 1)
    ...
    def linear2oklab__vec3(lin):
        lin = rt.copy(lin)
        l = rt.binary("+", rt.binary("+", rt.binary("*", rt.f(0.4122214708), rt.swizzle(lin, "r"), 1), rt.binary("*", rt.f(0.5363325363), rt.swizzle(lin, "g"), 1), 1), rt.binary("*", rt.f(0.0514459929), rt.swizzle(lin, "b"), 1), 1)
        m = rt.binary("+", rt.binary("+", rt.binary("*", rt.f(0.2119034982), rt.swizzle(lin, "r"), 1), rt.binary("*", rt.f(0.6806995451), rt.swizzle(lin, "g"), 1), 1), rt.binary("*", rt.f(0.1073969566), rt.swizzle(lin, "b"), 1), 1)
        s = rt.binary("+", rt.binary("+", rt.binary("*", rt.f(0.0883024619), rt.swizzle(lin, "r"), 1), rt.binary("*", rt.f(0.2817188376), rt.swizzle(lin, "g"), 1), 1), rt.binary("*", rt.f(0.6299787005), rt.swizzle(lin, "b"), 1), 1)
        l_ = rt.component_wise("pow", rt.component_wise("max", l, rt.f(0.0), width=1), rt.binary("/", rt.f(1.0), rt.f(3.0), 1), width=1)
        m_ = rt.component_wise("pow", rt.component_wise("max", m, rt.f(0.0), width=1), rt.binary("/", rt.f(1.0), rt.f(3.0), 1), width=1)
        s_ = rt.component_wise("pow", rt.component_wise("max", s, rt.f(0.0), width=1), rt.binary("/", rt.f(1.0), rt.f(3.0), 1), width=1)
        return rt.construct(3, rt.binary("-", rt.binary("+", rt.binary("*", rt.f(0.2104542553), l_, 1), rt.binary("*", rt.f(0.7936177850), m_, 1), 1), rt.binary("*", rt.f(0.0040720468), s_, 1), 1), rt.binary("+", rt.binary("-", rt.binary("*", rt.f(1.9779984951), l_, 1), rt.binary("*", rt.f(2.4285922050), m_, 1), 1), rt.binary("*", rt.f(0.4505937099), s_, 1), 1), rt.binary("-", rt.binary("+", rt.binary("*", rt.f(0.0259040371), l_, 1), rt.binary("*", rt.f(0.7827717662), m_, 1), 1), rt.binary("*", rt.f(0.8086757660), s_, 1), 1))
    ...
    def oklch2rgb__vec3(lch):
        lch = rt.copy(lch)
        a = rt.binary("*", rt.swizzle(lch, "y"), rt.component_wise("cos", rt.binary("*", rt.swizzle(lch, "z"), g.TAU, 1), width=1), 1)
        b = rt.binary("*", rt.swizzle(lch, "y"), rt.component_wise("sin", rt.binary("*", rt.swizzle(lch, "z"), g.TAU, 1), width=1), 1)
    ...
    def rgb2oklch__vec3(rgb):
        ...
        return rt.construct(3, rt.swizzle(lab, "x"), C, rt.component_wise("fract", rt.binary("/", h, g.TAU, 1), width=1))
    ...
    def getPosition__int_int(index, count):
        if rt.binary("==", _u_positionMode, rt.i(0)):
            return rt.binary("/", index, rt.construct(1, rt.binary("-", count, rt.i(1), 1)), 1)
        if rt.binary("==", index, rt.i(0)):
            return _u_pos0
        if rt.binary("==", index, rt.i(1)):
            return _u_pos1
    ...
    def mixInColorSpace__vec3_vec3_float_int(a, b, f, mode):
        ...
        b = rt.copy(b)
        if rt.binary("==", mode, rt.i(1)):
            dh = rt.binary("-", rt.swizzle(b, "x"), rt.swizzle(a, "x"), 1)
            if rt.binary(">", dh, rt.f(0.5)):
                dh = rt.binary("-", dh, rt.f(1.0), 1)
            if rt.binary("<", dh, rt.unary("-", rt.f(0.5))):
                dh = rt.binary("+", dh, rt.f(1.0), 1)
            return rt.construct(3, rt.component_wise("fract", rt.binary("+", rt.swizzle(a, "x"), rt.binary("*", dh, f, 1), 1), width=1), rt.component_wise("mix", rt.swizzle(a, "y"), rt.swizzle(b, "y"), f, width=1), rt.component_wise("mix", rt.swizzle(a, "z"), rt.swizzle(b, "z"), f, width=1))
        else:
            if rt.binary("==", mode, rt.i(3)):
                dh = rt.binary("-", rt.swizzle(b, "z"), rt.swizzle(a, "z"), 1)
                if rt.binary(">", dh, rt.f(0.5)):
                    dh = rt.binary("-", dh, rt.f(1.0), 1)
                if rt.binary("<", dh, rt.unary("-", rt.f(0.5))):
                    dh = rt.binary("+", dh, rt.f(1.0), 1)
                return rt.construct(3, rt.component_wise("mix", rt.swizzle(a, "x"), rt.swizzle(b, "x"), f, width=1), rt.component_wise("mix", rt.swizzle(a, "y"), rt.swizzle(b, "y"), f, width=1), rt.component_wise("fract", rt.binary("+", rt.swizzle(a, "z"), rt.binary("*", dh, f, 1), 1), width=1))
    ...
    def sampleColorArray__float_int_float(t, count, smoothAmount):
        ...
        for _for0 in range(1048576):
            ...
            bw = rt.f(0.0)
            if rt.binary("==", _u_positionMode, rt.i(0)):
                boundary = rt.binary("/", i, count, 1)
                bw = rt.binary("/", rt.binary("*", smoothAmount, rt.f(0.5), 1), count, 1)
            else:
                pPrev = getPosition__int_int(rt.binary("-", i, rt.i(1), 1), count)
                pCurr = getPosition__int_int(i, count)
                boundary = rt.binary("*", rt.binary("+", pPrev, pCurr, 1), rt.f(0.5), 1)
                bw = rt.binary("*", rt.binary("*", smoothAmount, rt.binary("-", pCurr, pPrev, 1), 1), rt.f(0.25), 1)
            blend = rt.component_wise("smoothstep", rt.binary("-", boundary, bw, 1), rt.binary("+", boundary, bw, 1), t, width=1)
        ...
        if rt.binary(">", smoothAmount, rt.f(0.0)):
            ...
            if rt.binary("==", _u_positionMode, rt.i(0)):
                ...
                pLast = getPosition__int_int(rt.binary("-", count, rt.i(1), 1), count)
                pFirst = getPosition__int_int(rt.i(0), count)
                gap = rt.binary("+", rt.binary("-", rt.f(1.0), pLast, 1), pFirst, 1)
                bw = rt.binary("*", rt.binary("*", smoothAmount, gap, 1), rt.f(0.25), 1)
            if rt.binary(">", bw, rt.f(0.0)):
                d = (rt.binary("-", t, rt.f(1.0), 1) if rt.binary(">", t, rt.f(0.5)) else t)
                wrapFactor = rt.component_wise("smoothstep", rt.unary("-", bw), bw, d, width=1)
                lastColor = rgbToColorSpace__vec3_int(getColor__int(rt.binary("-", count, rt.i(1), 1)), mode)
                ...
                wrapMask = rt.binary("-", rt.f(1.0), rt.component_wise("smoothstep", rt.f(0.0), bw, rt.component_wise("abs", d, width=1), width=1), 1)
    ...
    def main__void():
        ...
        t = rt.binary("+", rt.binary("*", rt.binary("*", lum, rt.binary("-", rt.f(1.0), rt.f(1e-4), 1), 1), _u_repeat, 1), _u_offset, 1)
